        ...
        for idx, layer in enumerate(self.features):
            if idx < 2:
                ...
                if idx == 1:
                    # x = x + PET + CT
                    num = x.shape[1] // 2
                    PETCT1, PETCT2 = torch.split(x, (num, num), dim=1)
                    PET1, PET2 = torch.split(PET, (num, num), dim=1)
                    CT1, CT2 = torch.split(CT, (num, num), dim=1)
                    x = torch.cat([PETCT1, PET1, CT1, PETCT2, PET2, CT2], dim=1)
                    x = self.blocks1(x)
                    PETCT1, PET1, CT1, PETCT2, PET2, CT2 = torch.split(x, (num, num, num, num, num, num), dim=1)
                    x = torch.cat([PETCT1, PETCT2], dim=1)
                    PET = torch.cat([PET1, PET2], dim=1)
                    CT = torch.cat([CT1, CT2], dim=1)
                    # print("xA", x.shape)
            elif 2 <= idx <4:
                if self.use_checkpoint:
                    x = checkpoint.checkpoint(layer, x)
