...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=16, stride=16, padding=0,
                 in_chans=3, embed_dim=768, norm_layer=None):
        super().__init__()
        # 错误expected stride to be a single integer value or a list of 2 values to match the convolution dimensions, but got stride=[4, 4, 4]
        # 原来是下面这三行代码导致的错误
        # patch_size = to_2tuple(patch_size)
        # stride = to_2tuple(stride)
        # padding = to_2tuple(padding)
...
#         self.proj = nn.Conv3d(in_chans, embed_dim, kernel_size=patch_size,
#                               stride=stride, padding=padding)
#         stride = ((stride[1]//2, stride[1], stride[1]))
#         stride = [(int(stride[1]//2), int(stride[1]), int(stride[1]))]
#         stride = int(stride[1])       # stride 4        # stride 2        # stride 2        # stride 2
#         print("stride", stride)
#         if int(stride[1]) == 4:
#             stride = ((stride[1]//4, stride[1], stride[1]))
#         else:
#             stride = ((stride[1], stride[1], stride[1]))
        ...
        # 继续错误input image (T: 2 H: 7 W: 7) smaller than kernel size (kT: 3 kH: 3 kW: 3)
            ...
            patch_size = ((patch_size//4, patch_size, patch_size))
        #     x torch.Size([1, 128, 10, 28, 28])        # x torch.Size([1, 320, 5, 14, 14])均需要修改
            padding = ((padding//4, padding, padding))
        else:
            #  input image (T: 2 H: 7 W: 7) smaller than kernel size (kT: 3 kH: 3 kW: 3)
            # 还需要修改，写个计数
            if in_chans == 320:
                stride = ((stride//2, stride, stride))
                patch_size = ((patch_size//2, patch_size, patch_size))
            #     x torch.Size([1, 128, 10, 28, 28])        # x torch.Size([1, 320, 5, 14, 14])均需要修改
            ...
            #     padding = ((padding//2, padding, padding))
        self.proj = nn.Conv3d(in_chans, embed_dim, kernel_size=patch_size,
                              stride=(stride), padding=padding)       # ut got stride=[2, 4, 4]
        self.norm = norm_layer(embed_dim) if norm_layer else nn.Identity()

    def forward(self, x):
        # print("x", x.shape)     # x torch.Size([1, 3, 16, 224, 224])
        x = self.proj(x)
        x = self.norm(x)
        return x
