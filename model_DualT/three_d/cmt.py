...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.conv1 = nn.Sequential(
            ...
            nn.GELU(),
            nn.BatchNorm3d(hidden_features, eps=1e-5),
        )
        self.proj = nn.Conv3d(hidden_features, hidden_features, 3, 1, 1, groups=hidden_features)
        self.proj_act = nn.GELU()
        self.proj_bn = nn.BatchNorm3d(hidden_features, eps=1e-5)
        self.conv2 = nn.Sequential(
            nn.Conv3d(hidden_features, out_features, 1, 1, 0, bias=True),
            nn.BatchNorm3d(out_features, eps=1e-5),
        )
        self.drop = nn.Dropout(drop)

    def forward(self, x, H, W):
        B, N, C = x.shape
        x = x.permute(0, 2, 1).reshape(B, C, -1, H, W)
        x = self.conv1(x)
        x = self.drop(x)
        x = self.proj(x) + x
        x = self.proj_act(x)
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None,
        ...
        if self.sr_ratio > 1:
            self.sr = nn.Sequential(
                # by YXY
                # nn.Conv3d(dim, dim, kernel_size=(sr_ratio), stride=sr_ratio, groups=dim, bias=True),
                nn.Conv3d(dim, dim, kernel_size=(sr_ratio//2, sr_ratio, sr_ratio), stride=sr_ratio, groups=dim, bias=True),
    ...
    def forward(self, x, H, W, relative_pos):
        ...
        if self.sr_ratio > 1:
            ...
            # by YXY
            x_ = x.permute(0, 2, 1).reshape(B, C, -1, H, W)
        ...
        # print("relative_pos", relative_pos.shape)     # q torch.Size([1, 1, 12544, 46])
        # print("q", q.shape)     # q torch.Size([1, 1, 12544, 46])
        # print("k.transpose(-2, -1)", k.transpose(-2, -1).shape)     # torch.Size([1, 1, 46, 49])
        # print("self.scale", self.scale)     # q torch.Size([1, 1, 12544, 46])
        # print("(q @ k.transpose(-2, -1))", (q @ k.transpose(-2, -1)).shape)     # torch.Size([1, 1, 12544, 49])
        # attn = (q @ k.transpose(-2, -1)) * self.scale + relative_pos
        # # by YXY
        qk = (q @ k.transpose(-2, -1)) * self.scale
        # print("qk", qk.shape)     # qk torch.Size([1, 1, 12544, 49])
        relative_pos = nn.Parameter(torch.randn(qk.shape[0], qk.shape[1], qk.shape[2], qk.shape[3]))
        relative_pos = relative_pos.cuda()
        attn = qk + relative_pos
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.proj = nn.Conv3d(dim, dim, 3, 1, 1, groups=dim)

    def forward(self, x, H, W, relative_pos):
        B, N, C = x.shape
        cnn_feat = x.permute(0, 2, 1).reshape(B, C, -1, H, W)
        x = self.proj(cnn_feat) + cnn_feat
        x = x.flatten(2).permute(0, 2, 1)
        x = x + self.drop_path(self.attn(self.norm1(x), H, W, relative_pos))
        x = x + self.drop_path(self.mlp(self.norm2(x), H, W))
        #         # by YXY
        # x = x.reshape(B, -1, H, W, C).permute(0, 4, 1, 2, 3)
        return x
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        # patch_size = to_2tuple(patch_size)
        #  by YXY
        #   File "E:\05DenseNet_Dense_SA\model_DualT\three_d\cmt.py", line 134, in forward
        #     attn = (q @ k.transpose(-2, -1)) * self.scale + relative_pos
        # RuntimeError: The size of tensor a (12544) must match the size of tensor b (3136) at non-singleton dimension 2

        num_patches = (img_size // patch_size) * (img_size // patch_size)
        # 这里的16是指输入特征图，是224*224*16
        # 但根据运行，这里需要填写8    The size of tensor a (49) must match the size of tensor b (196) at non-singleton dimension 3
        # 填写4   The size of tensor a (49) must match the size of tensor b (98) at non-singleton dimension 3
        # 填写2    (12544) must match the size of tensor b (3136) at non-si
        # num_patches = (img_size // patch_size) * (img_size // patch_size) * (2 // patch_size // patch_size)

        assert img_size % patch_size == 0 and img_size % patch_size == 0, \
            f"img_size {img_size} should be divided by patch_size {patch_size}."

        self.patch_sizeHW = patch_size
        # print("in_chans", in_chans)

        padding = (0, 0 ,0)
        # if int(patch_size) == 4:
        if in_chans == 3:
            # by YXY     发现这里多半是padding导致的错误，进行修改，这里也可能是卷积核导致的
        # 继续错误input image (T: 2 H: 7 W: 7) smaller than kernel size (kT: 3 kH: 3 kW: 3)
            self.patch_sizeT = patch_size//4
            num_patches = num_patches * patch_size
            # padding = ((patch_size // 4, patch_size, patch_size))
            patch_sizeA = ((1, patch_size, patch_size))
            kernel_size = ((1, patch_size, patch_size))
            # by YXY
        # elif in_chans == 46:
        #     patch_sizeA = ((patch_size // 2, patch_size, patch_size))
        #     kernel_size = ((patch_size, patch_size, patch_size))
        # elif in_chans == 46:
        #     self.patch_sizeT = patch_size//2
        #     patch_sizeA = ((patch_size // 2, patch_size, patch_size))
        #     # padding = ((patch_size // 2, patch_size, patch_size))
        #     kernel_size = ((patch_size, patch_size, patch_size))
        elif in_chans == 16:
            self.patch_sizeT = patch_size//2
            patch_sizeA = ((1, patch_size, patch_size))
        ...
        #     kernel_size = ((1, patch_size, patch_size))
            ...
            self.patch_sizeT = patch_size
            # 不能改为下面的：
            # self.patch_sizeT = patch_size//2
            # padding = ((patch_size, patch_size, patch_size))
            patch_sizeA = ((2, patch_size, patch_size))
            kernel_size = ((2, patch_size, patch_size))
            # patch_sizeA = ((patch_size // 2, patch_size, patch_size))
            # kernel_size = ((patch_size // 2, patch_size, patch_size))

        self.img_size = img_size
        self.patch_size = patch_size
        self.num_patches = num_patches

        self.proj = nn.Conv3d(in_chans, embed_dim, kernel_size=kernel_size, padding=padding, stride=patch_sizeA)
        self.norm = nn.LayerNorm(embed_dim)
    ...
    def forward(self, x):
        ...
        T, H, W = T // self.patch_sizeT, H // self.patch_sizeHW, W // self.patch_sizeHW
        return x, (T, H, W)
...
class CMT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=1000, embed_dims=[46, 92, 184, 368], stem_channel=16,
        ...
        # self.stem_conv1 = nn.Conv3d(3, stem_channel, kernel_size=3, stride=2, padding=1, bias=True)
        # by YXY
        self.stem_conv1 = nn.Conv3d(3, stem_channel, kernel_size=3, stride=(1, 2, 2), padding=1, bias=True)
        self.stem_relu1 = nn.GELU()
        self.stem_norm1 = nn.BatchNorm3d(stem_channel, eps=1e-5)

        self.stem_conv2 = nn.Conv3d(stem_channel, stem_channel, kernel_size=3, stride=1, padding=1, bias=True)
        self.stem_relu2 = nn.GELU()
        self.stem_norm2 = nn.BatchNorm3d(stem_channel, eps=1e-5)

        self.stem_conv3 = nn.Conv3d(stem_channel, stem_channel, kernel_size=3, stride=1, padding=1, bias=True)
        self.stem_relu3 = nn.GELU()
        self.stem_norm3 = nn.BatchNorm3d(stem_channel, eps=1e-5)
        ...
        # relative_pos torch.Size([1, 3136, 49])
        ...
        # Classifier head
        self._fc = nn.Conv3d(embed_dims[-1], fc_dim, kernel_size=1)
        self._bn = nn.BatchNorm3d(fc_dim, eps=1e-5)
...
def resize_pos_embed(posemb, posemb_new):
    ...
    posemb_grid = posemb_grid.reshape(1, gs_old, gs_old, -1).permute(0, 4, 1, 2, 3)
