...
class ConvRelPosEnc(nn.Module):
    ...
    def __init__(self, Ch, h, window):
        ...
        for cur_window, cur_head_split in window.items():
            ...
            # cur_conv = nn.Conv3d(cur_head_split*Ch, cur_head_split*Ch,
            ...
            # by YXY
            cur_conv = nn.Conv3d(cur_head_split*Ch, cur_head_split*Ch,
                kernel_size=(cur_window, cur_window, cur_window),
                padding=(padding_size, padding_size, padding_size),
    ...
    def forward(self, q, v, size):
        ...
        v_img = rearrange(v_img, 'B h (T H W) Ch -> B (h Ch) T H W', H=H, W=W)               # Shape: [B, h, H*W, Ch] -> [B, h*Ch, H, W].
        ...
        conv_v_img = rearrange(conv_v_img, 'B (h Ch) T H W -> B h (T H W) Ch', h=h)          # Shape: [B, h*Ch, H, W] -> [B, h, H*W, Ch].
...
class ConvPosEnc(nn.Module):
    ...
    def __init__(self, dim, k=3):
        ...
        # self.proj = nn.Conv3d(dim, dim, k, 1, k//2, groups=dim)
        # by YXY
        self.proj = nn.Conv3d(dim, dim, k, 1, k//2, groups=dim) 
    ...
    def forward(self, x, size):
        ...
        feat = img_tokens.transpose(1, 2).view(B, C, T, H, W)
...
class ParallelBlock(nn.Module):
    ...
    def interpolate(self, x, output_size, size):
        ...
        img_tokens = img_tokens.transpose(1, 2).reshape(B, C, T, H, W)
        # by YXY
        # img_tokens = F.interpolate(img_tokens, size=output_size, mode='bilinear')  # FIXME: May have alignment issue.
        img_tokens = F.interpolate(img_tokens, size=output_size, mode='trilinear')  # FIXME: May have alignment issue.
    ...
    def forward(self, x1, x2, x3, x4, sizes):
        _, (T2, H2, W2), (T3, H3, W3), (T4, H4, W4) = sizes
        
        # Conv-Attention.
        x2 = self.cpes[1](x2, size=(T2, H2, W2))  # Note: x1 is ignored.
        x3 = self.cpes[2](x3, size=(T3, H3, W3))
        x4 = self.cpes[3](x4, size=(T4, H4, W4))
        
        cur2 = self.norm12(x2)
        cur3 = self.norm13(x3)
        cur4 = self.norm14(x4)
        cur2 = self.factoratt_crpe2(cur2, size=(T2, H2, W2))
        cur3 = self.factoratt_crpe3(cur3, size=(T3, H3, W3))
        cur4 = self.factoratt_crpe4(cur4, size=(T4, H4, W4))
        upsample3_2 = self.upsample(cur3, output_size=(T2, H2, W2), size=(T3, H3, W3))
        upsample4_3 = self.upsample(cur4, output_size=(T3, H3, W3), size=(T4, H4, W4))
        upsample4_2 = self.upsample(cur4, output_size=(T2, H2, W2), size=(T4, H4, W4))
        downsample2_3 = self.downsample(cur2, output_size=(T3, H3, W3), size=(T2, H2, W2))
        downsample3_4 = self.downsample(cur3, output_size=(T4, H4, W4), size=(T3, H3, W3))
        downsample2_4 = self.downsample(cur2, output_size=(T4, H4, W4), size=(T2, H2, W2))
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, patch_size=16, in_chans=3, embed_dim=768):
        ...
        # 继续错误input image (T: 2 H: 7 W: 7) smaller than kernel size (kT: 3 kH: 3 kW: 3)
            patch_size = ((stride//4, stride, stride))
        else:
            # #  input image (T: 2 H: 7 W: 7) smaller than kernel size (kT: 3 kH: 3 kW: 3)
            # # 还需要修改，写个计数
            # if in_chans == 320:
            #     patch_size = ((stride//2, stride, stride))
            # else:
            #     stride = ((stride, stride, stride))
            patch_size = ((stride, stride, stride))
    ...
    def forward(self, x):
        ...
        # out_H, out_W = H // self.patch_size[0], W // self.patch_size[1]
        # by YXY
        out_T, out_H, out_W = T // self.patch_size[0], H // self.patch_size[1], W // self.patch_size[2]
...
class CoaT(nn.Module):
    ...
    def forward_features(self, x0):
        ...
        # Serial blocks 1.
        x1, (T1, H1, W1) = self.patch_embed1(x0)
        x1 = self.insert_cls(x1, self.cls_token1)
        for blk in self.serial_blocks1:
            x1 = blk(x1, size=(T1, H1, W1))
        x1_nocls = self.remove_cls(x1)
        #  by YXY
        x1_nocls = x1_nocls.reshape(B, T1, H1, W1, -1).permute(0, 4, 1, 2, 3).contiguous()
        
        # Serial blocks 2.
        x2, (T2, H2, W2) = self.patch_embed2(x1_nocls)
        x2 = self.insert_cls(x2, self.cls_token2)
        for blk in self.serial_blocks2:
            x2 = blk(x2, size=(T2, H2, W2))
        x2_nocls = self.remove_cls(x2)
        x2_nocls = x2_nocls.reshape(B, T2, H2, W2, -1).permute(0, 4, 1, 2, 3).contiguous()

        # Serial blocks 3.
        x3, (T3, H3, W3) = self.patch_embed3(x2_nocls)
        x3 = self.insert_cls(x3, self.cls_token3)
        for blk in self.serial_blocks3:
            x3 = blk(x3, size=(T3, H3, W3))
        x3_nocls = self.remove_cls(x3)
        x3_nocls = x3_nocls.reshape(B, T3, H3, W3, -1).permute(0, 4, 1, 2, 3).contiguous()

        # Serial blocks 4.
        x4, (T4, H4, W4) = self.patch_embed4(x3_nocls)
        x4 = self.insert_cls(x4, self.cls_token4)
        for blk in self.serial_blocks4:
            x4 = blk(x4, size=(T4, H4, W4))
        x4_nocls = self.remove_cls(x4)
        x4_nocls = x4_nocls.reshape(B, T4, H4, W4, -1).permute(0, 4, 1, 2, 3).contiguous()
        ...
        for blk in self.parallel_blocks:
            x1, x2, x3, x4 = blk(x1, x2, x3, x4, sizes=[(H1, W1), (T2, H2, W2), (T3, H3, W3), (T4, H4, W4)])
