...
def build_collate_fn(opts, *args, **kwargs):
    collate_fn_name_train = getattr(
        opts, "model_DualT.two_d.cvnets.dataset.collate_fn_name_train", "default_collate_fn"
    )
    collate_fn_name_val = getattr(
        opts, "model_DualT.two_d.cvnets.dataset.collate_fn_name_val", "default_collate_fn"
    )
...
def build_eval_collate_fn(opts, *args, **kwargs):
    collate_fn_name_eval = getattr(opts, "model_DualT.two_d.cvnets.dataset.collate_fn_name_eval", None)
...
for file in os.listdir(collate_fn_dir):
    ...
    if (
        ...
        module = importlib.import_module("model_DualT.two_d.cvnets.data.collate_fns." + collate_fn_fname)
