...
from model_DualT.two_d.cvnets.utils.ddp_utils import is_master
...
def register_dataset(name, task):
    def register_dataset_class(cls):
        ...
        if not issubclass(cls, BaseImageDataset):
            raise ValueError(
                "model_DualT.two_d.cvnets.dataset ({}: {}) must extend BaseImageDataset".format(
                    name, cls.__name__
                )
            )
...
def supported_dataset_str(dataset_name, dataset_category):
    ...
    supp_str = "model_DualT.two_d.cvnets.dataset ({}) under task ({}) is not yet supported. \n Supported datasets are:".format(
...
def evaluation_datasets(opts):
    dataset_name = getattr(opts, "model_DualT.two_d.cvnets.dataset.name", "imagenet")
    dataset_category = getattr(opts, "model_DualT.two_d.cvnets.dataset.category", "classification")
...
def train_val_datasets(opts):
    dataset_name = getattr(opts, "model_DualT.two_d.cvnets.dataset.name", "imagenet")
    dataset_category = getattr(opts, "model_DualT.two_d.cvnets.dataset.category", "classification")
    disable_val = getattr(opts, "model_DualT.two_d.cvnets.dataset.disable_val", False)
...
def general_dataset_args(parser: argparse.ArgumentParser):
    group = parser.add_argument_group(
        title="model_DualT.two_d.cvnets.dataset", description="Arguments related to dataset"
    )
    group.add_argument(
        "--dataset.root-train",
        type=str,
    ...
    group.add_argument(
        "--dataset.name", type=str, default="imagenet", help="model_DualT.two_d.cvnets.dataset name"
    )
    group.add_argument(
        "--dataset.category",
        type=str,
        default="classification",
        help="model_DualT.two_d.cvnets.dataset category (e.g., segmentation, classification)",
    )
    group.add_argument(
    ...
    for file in os.listdir(task_path):
        ...
        if (
            ...
            module = importlib.import_module(
                "model_DualT.two_d.cvnets.data.datasets." + task + "." + dataset_name
            )
