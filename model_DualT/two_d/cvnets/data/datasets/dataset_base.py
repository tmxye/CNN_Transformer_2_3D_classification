...
from model_DualT.two_d.cvnets.utils.ddp_utils import is_start_rank_node, is_master
...
class BaseImageDataset(data.Dataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if getattr(opts, "model_DualT.two_d.cvnets.dataset.trove.enable", False):
            opts = self.load_from_server(opts=opts, is_training=is_training)

        root = (
            getattr(opts, "model_DualT.two_d.cvnets.dataset.root_train", None)
            if is_training
            else getattr(opts, "model_DualT.two_d.cvnets.dataset.root_val", None)
        )
        self.root = root
        ...
        image_device_cuda = getattr(self.opts, "model_DualT.two_d.cvnets.dataset.decode_data_on_gpu", False)
        ...
        if use_cuda and getattr(opts, "model_DualT.two_d.cvnets.dataset.pin_memory", False):
            if is_master(opts):
                logger.error(
        ...
        self.cached_data = (
            dict()
            if getattr(opts, "model_DualT.two_d.cvnets.dataset.cache_images_on_ram", False) and is_training
            else None
        )
        if self.cached_data is not None:
            if not getattr(opts, "model_DualT.two_d.cvnets.dataset.persistent_workers", False):
                if is_master(opts):
                    logger.error(
                        "For caching, --dataset.persistent-workers should be enabled."
                    )

        self.cache_limit = getattr(opts, "model_DualT.two_d.cvnets.dataset.cache_limit", 80.0)
    ...
    def load_from_server(opts, is_training):
        ...
        mount_path = getattr(opts, "model_DualT.two_d.cvnets.dataset.trove.mount_path", "/mnt/vision_datasets")
        disk_cache_max_size_gb = getattr(
            opts, "model_DualT.two_d.cvnets.dataset.trove.disk_cache_max_size_gb", None
        )
        disk_cache_mount_size_gb = getattr(
            opts, "model_DualT.two_d.cvnets.dataset.trove.disk_cache_mount_size_gb", None
        )
        disk_cache_dir = getattr(
            opts, "model_DualT.two_d.cvnets.dataset.trove.disk_cache_dir", "/mnt/trove_cache/"
        )
        trove_uri = getattr(opts, "model_DualT.two_d.cvnets.dataset.trove.uri", None)
        ...
        if is_training:
            path_prefix = getattr(opts, "model_DualT.two_d.cvnets.dataset.trove.dir_train", None)
            if path_prefix is None:
                logger.error(
                    "Path prefix cannot be None. Please specify using --dataset.trove.train-path-prefix"
                )
            setattr(opts, "model_DualT.two_d.cvnets.dataset.root_train", "{}/{}".format(mount_path, path_prefix))
        else:
            path_prefix = getattr(opts, "model_DualT.two_d.cvnets.dataset.trove.dir_val", None)
            if path_prefix is not None:
                setattr(
                    opts, "model_DualT.two_d.cvnets.dataset.root_val", "{}/{}".format(mount_path, path_prefix)
                )
            else:
                setattr(opts, "model_DualT.two_d.cvnets.dataset.root_val", None)
