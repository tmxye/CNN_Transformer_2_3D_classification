...
class MobileVitBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        global_repr = rearrange(local_repr, 'b d (h ph) (w pw) -> b (ph pw) (h w) d', ph=1, pw=1)
        global_repr = self.transformer(global_repr)
        global_repr = rearrange(global_repr, 'b (ph pw) (h w) d -> b d (h ph) (w pw)', h=h, w=w, ph=1, pw=1)
...
class MobileVitBlock2(nn.Module):
    def __init__(self, in_channels, out_channels, d_model, layers, mlp_dim):
        super(MobileVitBlock2, self).__init__()
        # Local representation
        self.local_representation = nn.Sequential(
            # Encode local spatial information
            ConvNormAct(in_channels, in_channels, 3),
...
class MobileViT(nn.Module):
    def __init__(self, img_size, features_list, d_list, transformer_depth, expansion, num_classes = 1000):
        ...
        self.stage4 = nn.Sequential(
            ...
            MobileVitBlock2(in_channels = features_list[8], out_channels = features_list[9], d_model = d_list[2],
