...
class PfAAMLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        yy = x_minus_mu_square / (4 * (x_minus_mu_square.sum(dim=[2, 3, 4], keepdim=True) / n + self.e_lambda)) + 0.5


        # return x * self.sigmoid(y * z * s)
        # return x * self.activaton(yy) * self.sigmoid(y * z * s)
        return x * self.sigmoid(y * z * s + yy)
...
class Cross_Attention_pure(nn.Module):
    def __init__(self, dim=8, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0.1, proj_drop=0.):
        super().__init__()
        self.num_heads = num_heads
...
class GhostModuleMul(nn.Module):
    def __init__(self, num_in, num_out, kernel_size=1, stride=1, padding=0, ratio=2, dw_size=3,
        ...
        self.avgpool2d = nn.AvgPool3d(kernel_size=(2,2,2), stride=(2,2,2))
        self.gate_fn = nn.Sigmoid()
        self.short_conv = nn.Sequential(
            nn.Conv3d(num_in, num_out, kernel_size=(1,kernel_size,kernel_size), stride=(1,1,1),
                     padding=(0,kernel_size//2,kernel_size//2), groups=1),
            nn.Conv3d(num_out, num_out, kernel_size=(1, 1, 5), stride=(1,1,1),
                     padding=(0, 0, 2), groups=num_out),
            nn.Conv3d(num_out, num_out, kernel_size=(1, 5, 1), stride=(1,1,1),
                     padding=(0, 2, 0), groups=num_out),
            )
    ...
    def forward(self, x):
        ...
        # out = F.interpolate(res, size=x.shape[-2:], mode="bilinear", align_corners=True)
        out = F.interpolate(res, size=x.shape[-3:], mode="trilinear", align_corners=True)
        return out


class GhostModuleMulA(nn.Module):
    def __init__(self, num_in, num_out, kernel_size=1, stride=1, padding=0, ratio=2, dw_size=3,
                 use_act=True, act_type='relu'):
        super(GhostModuleMulA, self).__init__()
        self.avgpool2d = nn.AvgPool3d(kernel_size=(2,2,2), stride=(2,2,2))
        self.gate_fn = nn.Sigmoid()
        self.short_conv = nn.Sequential(
            nn.Conv3d(num_in, num_out, kernel_size=(1,kernel_size,kernel_size), stride=(1,1,1),
...
class MobileViT(nn.Module):
    def __init__(self, img_size, features_list, d_list, transformer_depth, expansion, num_classes = 1000):
        ...
        self.stem = nn.Sequential(
            nn.Conv3d(in_channels = 3, out_channels = features_list[2], kernel_size = 3, stride = 4, padding = 1),
        ...
        # self.cross_conv1 = nn.Identity()
        self.cross_conv1 = nn.Conv3d(features_list[2], features_list[2], 1, stride=1)
        self.GhostModuleMul1 = GhostModuleMul(features_list[2], features_list[2])
        self.cross_attn1 = Cross_Attention_pure()
        ...
        self.stage1 = nn.Sequential(
            ...
            MobileVitBlock(in_channels=features_list[2], out_channels=features_list[3], d_model=d_list[0] // 4,
                           layers=transformer_depth[0], mlp_dim=d_list[0] // 4, stride=2)
        )

        self.cross_conv2 = nn.Conv3d(features_list[2], features_list[3], 1, stride=2)
        self.GhostModuleMul2 = GhostModuleMul(features_list[3], features_list[3])
        self.cross_attn2 = Cross_Attention_pure()

        self.stage2 = nn.Sequential(
            # GhostBottleneck(features_list[3], features_list[4]//2, features_list[4], 3, 2, se_ratio=0),
            MobileVitBlock(in_channels = features_list[3], out_channels = features_list[5], d_model = d_list[0] // 2,
                           layers = transformer_depth[0], mlp_dim = d_list[0] // 2, stride=2)
        )

        self.cross_conv3 = nn.Conv3d(features_list[3], features_list[5], 1, stride=2)
        self.GhostModuleMul3 = GhostModuleMulA(features_list[5], features_list[5])
        self.cross_attn3 = Cross_Attention_pure()
        ...
        self.stage3 = nn.Sequential(
            ...
            MobileVitBlock(in_channels = features_list[5], out_channels = features_list[6], d_model = d_list[1],
                           layers = transformer_depth[1], mlp_dim = d_list[1] * 1, stride=1),
            MobileVitBlock(in_channels = features_list[6], out_channels = features_list[6], d_model = d_list[1],
                           layers = transformer_depth[1], mlp_dim = d_list[1] * 1, stride=1),
        ...
        self.cross_conv4 = nn.Conv3d(features_list[5], features_list[7], 1, stride=2)
        self.GhostModuleMul4 = GhostModuleMulA(features_list[7], features_list[7])
        self.cross_attn4 = Cross_Attention_pure()
        ...
        self.stage4 = nn.Sequential(
            ...
            MobileVitBlock(in_channels = features_list[7], out_channels = features_list[9], d_model = d_list[2],
        ...
        self.fc = nn.Linear(features_list[9], num_classes)
    ...
    def forward(self, x, PET=None, CT=None):
        ...
        # 第一阶段
        PET = self.cross_conv1(PET)
        CT = self.cross_conv1(CT)
        PET = PET * self.GhostModuleMul1(PET)
        CT = CT * self.GhostModuleMul1(CT)
        PET = PET + CT
        B_conv, C_conv, T_conv, H_conv, W_conv = x.shape
        N_conv = T_conv * H_conv * W_conv
        conv_q = x.permute(0, 2, 3, 4, 1).reshape(B_conv, N_conv, C_conv)
        sa_k = PET.permute(0, 2, 3, 4, 1).reshape(B_conv, N_conv, C_conv)
        sa_v = PET.permute(0, 2, 3, 4, 1).reshape(B_conv, N_conv, C_conv)
        x = self.cross_attn1(conv_q, sa_k, sa_v, shape=(B_conv, N_conv, C_conv))

        x = x.permute(0, 2, 1).reshape(B_conv, C_conv, T_conv, H_conv, W_conv)

        x = self.stage1(x)


        # 第二阶段
        PET = self.cross_conv2(PET)
        CT = self.cross_conv2(CT)
        PET = PET * self.GhostModuleMul2(PET)
        CT = CT * self.GhostModuleMul2(CT)
        PET = PET + CT
        B_conv, C_conv, T_conv, H_conv, W_conv = x.shape
        N_conv = T_conv * H_conv * W_conv
        conv_q = x.permute(0, 2, 3, 4, 1).reshape(B_conv, N_conv, C_conv)
        sa_k = PET.permute(0, 2, 3, 4, 1).reshape(B_conv, N_conv, C_conv)
        sa_v = PET.permute(0, 2, 3, 4, 1).reshape(B_conv, N_conv, C_conv)
        x = self.cross_attn2(conv_q, sa_k, sa_v, shape=(B_conv, N_conv, C_conv))
        ...
        PET = self.cross_conv3(PET)
        CT = self.cross_conv3(CT)
        PET = PET * self.GhostModuleMul3(PET)
        CT = CT * self.GhostModuleMul3(CT)
        PET = PET + CT
        B_conv, C_conv, T_conv, H_conv, W_conv = x.shape
        N_conv = T_conv * H_conv * W_conv
        conv_q = x.permute(0, 2, 3, 4, 1).reshape(B_conv, N_conv, C_conv)
        sa_k = PET.permute(0, 2, 3, 4, 1).reshape(B_conv, N_conv, C_conv)
        sa_v = PET.permute(0, 2, 3, 4, 1).reshape(B_conv, N_conv, C_conv)
        x = self.cross_attn3(conv_q, sa_k, sa_v, shape=(B_conv, N_conv, C_conv))
        ...
        PET = self.cross_conv4(PET)
        CT = self.cross_conv4(CT)
        PET = PET * self.GhostModuleMul4(PET)
        CT = CT * self.GhostModuleMul4(CT)
        PET = PET + CT
        B_conv, C_conv, T_conv, H_conv, W_conv = x.shape
        N_conv = T_conv * H_conv * W_conv
        conv_q = x.permute(0, 2, 3, 4, 1).reshape(B_conv, N_conv, C_conv)
        sa_k = PET.permute(0, 2, 3, 4, 1).reshape(B_conv, N_conv, C_conv)
        sa_v = PET.permute(0, 2, 3, 4, 1).reshape(B_conv, N_conv, C_conv)
        x = self.cross_attn4(conv_q, sa_k, sa_v, shape=(B_conv, N_conv, C_conv))
