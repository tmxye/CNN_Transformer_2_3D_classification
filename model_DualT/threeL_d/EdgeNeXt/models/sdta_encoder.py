...
class SDTAEncoder(nn.Module):
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6, expan_ratio=4,
        ...
        for i in range(self.nums):
            convs.append(nn.Conv3d(width, width, kernel_size=3, padding=1, groups=width))
    ...
    def forward(self, x):
        ...
        if self.pos_embd:
            # print("pos_embd is true 执行了")       # 确实执行了
            pos_encoding =  self.pos_embd(B, T, H, W).reshape(B, -1, x.shape[1]).permute(0, 2, 1)
            x = x + pos_encoding
