...
class MobileVitBlock(nn.Module):
    def __init__(self, in_channels, out_channels, d_model, layers, mlp_dim, stride):
        ...
        # by YYX, 2024 04
        self.transformerEfficient = EfficientAdditiveAttnetion(in_dims=out_channels // 4, token_dim=out_channels // 4, num_heads=1)
    ...
    def forward(self, x):
        ...
        b, c, t, h, w = local_repr.shape

        '''
        # self.attn(x.permute(0, 2, 3, 1).reshape(B, H * W, C)).reshape(B, H, W, C).permute(0, 3, 1, 2))
        local_reprA = local_repr.permute(0, 2, 3, 4, 1).reshape(b, t * h * w, c)
        global_reprA = self.transformerEfficient(local_reprA).reshape(b, t, h, w, c).permute(0, 4, 1, 2, 3)
        print("AAAA", global_reprA.size())
        ...
        # self.attn(x.permute(0, 2, 3, 1).reshape(B, H * W, C)).reshape(B, H, W, C).permute(0, 3, 1, 2))
        local_reprA = local_repr.permute(0, 2, 3, 4, 1).reshape(b, t * h * w, c)
        global_reprB = self.transformerEfficient(local_reprA)
        global_reprA = global_reprB.reshape(b, t, h, w, c).permute(0, 4, 1, 2, 3)
        global_reprB1 = self.transformerEfficient(global_reprB)
        global_reprA1 = global_reprB1.reshape(b, t, h, w, c).permute(0, 4, 1, 2, 3)
        global_reprA2 = self.transformerEfficient(global_reprB1).reshape(b, t, h, w, c).permute(0, 4, 1, 2, 3)

        result = torch.cat([local_repr, global_reprA, global_reprA1, global_reprA2], dim = 1)
...
model_cfg = {
    ...
    "xxs":{
        ...
        "features": [16, 96, 192, 64, 384, 768, 8, 48, 96, 64, 192, 384],
        "d": [64, 80, 96],
        "expansion_ratio": 2,
        # "layers": [1, 3, 2]
        "layers": [1, 1, 1]
...
class MHSA(nn.Module):
    def __init__(self, n_dims, width=7, height=7, heads=4):
    ...
    def forward(self, x, PETCT):
        n_batch, C, T, width, height = x.size()
        q = self.query(x).view(n_batch, self.heads, C // self.heads, -1)
        k = self.key(PETCT).view(n_batch, self.heads, C // self.heads, -1)
        v = self.value(PETCT).view(n_batch, self.heads, C // self.heads, -1)
...
class MobileViT(nn.Module):
    def __init__(self, img_size, features_list, d_list, transformer_depth, expansion, num_classes = 1000):
        ...
        self.blocks1 = MixBlock(dim=embed_dim[0], num_heads=num_heads[0], mlp_ratio=4., qkv_bias=True, qk_scale=None,
                drop=0, attn_drop=0, drop_path=0., norm_layer=nn.LayerNorm, downsample=downsamples[0])
        self.blocks2 = MixBlock(dim=embed_dim[1], num_heads=num_heads[1], mlp_ratio=4., qkv_bias=True, qk_scale=None,
                drop=0, attn_drop=0, drop_path=0., norm_layer=nn.LayerNorm, downsample=downsamples[1])
        self.blocks3 = MixBlock(dim=embed_dim[2], num_heads=num_heads[2], mlp_ratio=4., qkv_bias=True, qk_scale=None,
                drop=0, attn_drop=0, drop_path=0., norm_layer=nn.LayerNorm, downsample=downsamples[2])
        self.blocks4 = MixBlock(dim=embed_dim[3], num_heads=num_heads[3], mlp_ratio=4., qkv_bias=True, qk_scale=None,
                drop=0, attn_drop=0, drop_path=0., norm_layer=nn.LayerNorm, downsample=downsamples[3])


        self.block1 = MHSA(n_dims=96, width=56, height=56, heads=8)
        self.block2 = MHSA(n_dims=192, width=28, height=28, heads=8)
        self.block3 = MHSA(n_dims=384, width=14, height=14, heads=8)
        self.block4 = MHSA(n_dims=768, width=7, height=7, heads=8)
    ...
    def forward(self, x, PET=None, CT=None):
        ...
        PET = self.stemCP(PET)
        CT = self.stemCP(CT)
        # PETCT = torch.cat([PET, CT], dim=1)
        # Body
        x = self.stage1(x)
        PET = self.stage1CP(PET)
        CT = self.stage1CP(CT)
        PETCT = torch.cat([PET, CT], dim=1)
        x = self.block1(x, PETCT)
        PETCT = self.block1(PETCT, x)
        PET, CT = torch.split(PETCT, (48, 48), dim=1)

        x = self.stage2(x)
        PET = self.stage2CP(PET)
        CT = self.stage2CP(CT)
        PETCT = torch.cat([PET, CT], dim=1)
        x = self.block2(x, PETCT)
        PETCT = self.block2(PETCT, x)
        PET, CT = torch.split(PETCT, (96, 96), dim=1)

        x = self.stage3(x)
        PET = self.stage3CP(PET)
        CT = self.stage3CP(CT)
        PETCT = torch.cat([PET, CT], dim=1)
        x = self.block3(x, PETCT)
        PETCT = self.block3(PETCT, x)
        PET, CT = torch.split(PETCT, (192, 192), dim=1)

        x = self.stage4(x)
        PET = self.stage4CP(PET)
        CT = self.stage4CP(CT)
        PETCT = torch.cat([PET, CT], dim=1)
        x = self.block4(x, PETCT)
        PETCT = self.block4(PETCT, x)
        PET, CT = torch.split(PETCT, (384, 384), dim=1)
...
# 提高输出可读性Params(M)是        #  3.774M
# 提高输出可读性MACs(G)是        #  1.142G

# 提高输出可读性Params(M)是#  4.542M
# 提高输出可读性MACs(G)是#  1.142G