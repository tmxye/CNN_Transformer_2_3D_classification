...
class NAT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x_A, x_B, x_C, x_D = shortcut_A.split([int(x.shape[3]/4) * 1, int(x.shape[3]/4) * 1, int(x.shape[3]/4) * 1, int(x.shape[3]/4) * 1], dim=3)
        x_B = self.level2res_O(x_A)
        x_C = self.level2res_O(x_B + x_C)
        x_D = self.level2res_O(x_C + x_D)
...
class SEDenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        self.NAT = NAT(depths=[1], num_heads=[16], embed_dim=256, mlp_ratio=3, drop_path_rate=0.2, in_chans=num_features, kernel_size=7, num_classes=num_classes)
