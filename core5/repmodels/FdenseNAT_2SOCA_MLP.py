...
class NAT(nn.Module):
    def __init__(self, embed_dim, mlp_ratio, depths, num_heads, drop_path_rate=0.2, in_chans=3, kernel_size=7,
        ...
        self.num_levels = depths[0]
        self.level2res_O_MLP = NATBlock_MLP(dim=int(embed_dim),
                                  depth=1,
                                  num_heads=num_heads[i],
                                  kernel_size=kernel_size,
                                  mlp_ratio=self.mlp_ratio,
    ...
    def forward_features(self, x):
        ...
        shortcut_A = self.level2res_O_MLP(shortcut_A)
        ...
        shortcut_B = self.level2res_O_MLP(shortcut_B)
        ...
        shortcut_C = self.level2res_O_MLP(shortcut_C)
        ...
        shortcut_D = self.level2res_O_MLP(shortcut_D)
