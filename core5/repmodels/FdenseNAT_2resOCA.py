...
class ConvTokenizer(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("Bx", x.shape)     # ([16, 2, 2, 128])
        if self.norm is not None:
            x = self.norm(x)
            # print("Cx", x.shape)     # ([16, 2, 2, 128])
        return x
...
class NATLayer(nn.Module):
    ...
    def forward(self, x):
        # print("shortcutxAA", x.shape)         # torch.Size([16, 2, 2, 32])
        if not self.layer_scale:
            # torch.Size([16, 2, 2, 32])
            shortcut = x
            x = self.norm1(x)
            x = self.attn(x)
...
class NAT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        # print("Bx", x.shape)     # Bx torch.Size([1, 2, 2, 512])        # [16, 2, 2, 128])
        x = self.pos_drop(x)
        # print("Cx", x.shape)            # ([16, 2, 2, 128])


        # # x = x.view(x.shape[0], x.shape[3], x.shape[1], x.shape[2])
        # x = x.permute(0, 3, 1, 2)           # https://www.csdn.net/tags/MtTaEgwsOTA4MjM5LWJsb2cO0O0O.html
        # # permute中的参数是要进行维度交换的tensor中的维度索引，就是将第几维度放在第0，1，2，3维度上
        # # print("Cx", x.shape)            # ([2, 128, 2, 2])
        # out = self.conv3(x)
        # out = self.bn3(out)
        # out = self.relu3(out)
        # # print("out", out.shape)            # ([2, 128, 2, 2])
        # x = x.permute(0, 2, 3, 1)
        ...
        # x_C = self.level2res_O(x_B + x_C)
        # x_D = self.level2res_O(x_C + x_D)
        # shortcut_E = torch.cat((x_A, x_B, x_C, x_D), dim=3)
        # x = shortcut_D + shortcut_E


        # print("x", x.shape)     # ([2, 2, 2, 512])
        x = self.norm(x).flatten(1, 2)
        # print("Cx", x.shape)     # torch.Size([16, 4, 512])
        x = self.avgpool(x.transpose(1, 2))
        # print("Dx", x.shape)     # (([16, 512, 1])
        x = torch.flatten(x, 1)
        # print("Ex", x.shape)     # ( torch.Size([16, 512])
        return x
