...
from .Enatten import NeighborhoodAttention
...
class ConvDownsampler(nn.Module):
    def __init__(self, dim, norm_layer=nn.LayerNorm):
        ...
        self.reduction = nn.Conv2d(dim, dim, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False)
        self.norm = norm_layer(dim)

    def forward(self, x):
        x = self.reduction(x).permute(0, 2, 3, 1)
        print("x", x.shape)     # ([24, 7, 7, 432])
        x = self.norm(x)
        return x
...
class NAT(nn.Module):
    def __init__(self, embed_dim, mlp_ratio, depths, num_heads, drop_path_rate=0.2, in_chans=3, kernel_size=7,
        ...
        self.patch_embed = ConvDownsampler(dim=embed_dim, norm_layer=norm_layer)
        ...
        self.num_features = 432
        self.norm = norm_layer(self.num_features)
        self.avgpool = nn.AdaptiveAvgPool1d(1)
        self.head = nn.Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()

        self.apply(self._init_weights)
        # self.num_features = 1008
        # self.head = ClassifierHead(self.num_features, 1000, pool_type='avg', drop_rate=0.)
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)    # ([2, 7, 7, 432])
        # x = self.norm(x).flatten(1, 2)
        # x = self.avgpool(x.transpose(1, 2))
        # x = torch.flatten(x, 1)

        x = x.permute(0, 3, 1, 2)    # ([2, 432, 7, 7])
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_chs, out_chs, stride=1, dilation=1, bottleneck_ratio=1, group_width=1, se_ratio=0.25,
        ...
        if self.downsample is not None:
            self.convB = ConvBnAct(bottleneck_chs, bottleneck_chs, kernel_size=3, stride=stride, dilation=dilation, groups=groups, **cargs)
            # self.Pool = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
            # self.convB = conv_bn_relu(bottleneck_chs, bottleneck_chs, kernel_size=3, stride=1, dilation=dilation, groups=groups, padding=1)
        ...
        if self.downsample is not None:
            # self.convC = ConvBnAct(bottleneck_chs, out_chs, kernel_size=1, stride=stride, **cargs)
            self.convC = conv_bn(bottleneck_chs, out_chs, kernel_size=1)
        else:
            self.convC = conv_bn(bottleneck_chs, out_chs, kernel_size=1)
...
class RegNet(nn.Module):
    ...
    def __init__(self, cfg, in_chans=3, num_classes=2, output_stride=32, global_pool='avg', drop_rate=0.,
        ...
        block_in_chs = 432                 # 432
        self.downsample_avg = downsample_avg(in_chs=432, out_chs=block_in_chs, kernel_size=1)
        self.convA = conv_bn_relu(432, block_in_chs, kernel_size=1)
        self.convB = ConvBnAct(block_in_chs, block_in_chs, kernel_size=3, stride=2, dilation=1, groups=9)
        self.convC = conv_bn_relu(block_in_chs, block_in_chs, kernel_size=1)
        self.norm = nn.LayerNorm(block_in_chs)
        self.NAT = NAT(depths=[2], num_heads=[16], embed_dim=432, mlp_ratio=3, drop_path_rate=0.2, in_chans=block_in_chs, kernel_size=7, num_classes=num_classes)
        block_in_chs = 1008-432
        self.convA1 = conv_bn_relu(432, block_in_chs, kernel_size=1)
        # self.convB1 = ConvBnAct(block_in_chs, block_in_chs, kernel_size=3, stride=1, dilation=1, groups=21)
        self.convB1 = conv_bn_relu(block_in_chs, block_in_chs, kernel_size=3, stride=1, dilation=1, groups=12, padding=1)
        self.convC1 = conv_bn_relu(block_in_chs, block_in_chs, kernel_size=1)
        self.convA2 = conv_bn_relu(block_in_chs, block_in_chs, kernel_size=1)
        # self.convB2 = ConvBnAct(block_in_chs, block_in_chs, kernel_size=3, stride=2, dilation=1, groups=21)
        # self.convB2 = ConvBnAct(block_in_chs, block_in_chs, kernel_size=3, stride=2, dilation=1, groups=12)
        self.convB2 = conv_bn_relu(block_in_chs, block_in_chs, kernel_size=3, stride=2, dilation=1, groups=12, padding=1)
        self.convC2 = conv_bn_relu(block_in_chs, block_in_chs, kernel_size=1)


        # Construct the head
        self.num_features = prev_width
        # self.head = ClassifierHead(in_chs=prev_width, num_classes=num_classes, pool_type=global_pool, drop_rate=drop_rate)
        # self.head = ClassifierHead(in_chs=432, num_classes=num_classes, pool_type=global_pool, drop_rate=drop_rate)
        # self.head = ClassifierHead(in_chs=432+1008, num_classes=num_classes, pool_type=global_pool, drop_rate=drop_rate)
        self.head = ClassifierHead(in_chs=1008, num_classes=num_classes, pool_type=global_pool, drop_rate=drop_rate)
    ...
    def forward(self, x):
        # for block in self.children():
        #     x = block(x)
        # for block in list(self.children())[:5]:
        for block in list(self.children())[:4]:
        # for block in list(self.children())[:-7]:
            x = block(x)
            NAT = x
            # print("看看出现的次数", x.shape)        #     ([16, 432, 14, 14])
        ...
        x = self.convA(NAT)
        x = self.convB(x)
        x = self.convC(x).permute(0, 2, 3, 1)
