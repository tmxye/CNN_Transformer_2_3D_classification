...
class NATBlock(nn.Module):
    def __init__(self, dim, depth, num_heads, kernel_size, downsample=False,
...
class NAT(nn.Module):
    ...
    def forward_features(self, x):
        ...
        # for level2res in self.level2ress:
        #     print("循环几次")       # 只循环一次，也就是这里不能控制中间层数，由num_heads参数控制着
        #     x_A, x_B, x_C, x_D =  x.split([int(x.shape[3]/4) * 1, int(x.shape[3]/4) * 1, int(x.shape[3]/4) * 1, int(x.shape[3]/4) * 1], dim=3)
        #     x_A = level2res(x_A)
        #     x_B = level2res(x_A + x_B)
        #     x_C = level2res(x_B + x_C)
        #     x_D = level2res(x_C + x_D)
        ...
        for i in range(self.num_levels):
            ...
            x_A, x_B, x_C, x_D =  x.split([int(x.shape[3]/4) * 1, int(x.shape[3]/4) * 1, int(x.shape[3]/4) * 1, int(x.shape[3]/4) * 1], dim=3)
            # x_A = self.level2res_O(x_A)
            x_B = self.level2res_O(x_B)
            x_C = self.level2res_O(x_B + x_C)
            x_D = self.level2res_O(x_C + x_D)
            shortcut = torch.cat((x_A, x_B, x_C, x_D), dim=3)
            # x = x + shortcut
            x = self.normLN(shortcut)
...
class SEDenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        # self.NAT = NAT(depths=[4], num_heads=[16], embed_dim=256, mlp_ratio=3, drop_path_rate=0.2, in_chans=num_features, kernel_size=7, num_classes=num_classes)
        self.NAT = NAT(depths=[4], num_heads=[16], embed_dim=512, mlp_ratio=3, drop_path_rate=0.2, in_chans=num_features, kernel_size=7, num_classes=num_classes)
