...
class InceptionA_1_1_M(nn.Module):
    #输出尺寸是1536
    def __init__(self, input_channels, output_channels):
        #"""Figure 6. The schema for 8×8 grid modules of the pure
        #Inceptionv4 network. This is the Inception-C block of Figure 9."""
    ...
    def forward(self, x):
        branch3x3stack_output = self.branch3x3stack(x)
        branch3x3stack_output = self.MaxPooling(branch3x3stack_output)

        branch3x3_output = self.branch3x3(x)
        branch3x3_output = self.MaxPooling(branch3x3_output)

        branch1x1_output = self.branch1x1(x)
        branch1x1_output = self.MaxPooling(branch1x1_output)
...
class SEDenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        self.features = self._generate_inception_module(3, 64, 1, InceptionA_1_1_M)
