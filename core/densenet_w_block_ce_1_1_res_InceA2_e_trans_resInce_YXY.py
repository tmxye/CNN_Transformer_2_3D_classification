...
class InceptionA(nn.Module):
    ...
    def __init__(self, input_channels, output_channels):
        ...
        self.branch1x1 = BasicConv2d(input_channels, int(output_channels * 0.25), kernel_size=1,
                                     stride=2)  # 这里是256     256-----10.6667     待会用这个补  8

        self.branchpool = nn.Sequential(
            nn.AvgPool2d(kernel_size=3, stride=2, padding=1),
            BasicConv2d(input_channels, int(output_channels * 0.25), kernel_size=1)
            # 这里是256     256-----10.6667         待会用这个补    8
...
class InceptionA_1_5_res(nn.Module):
    ...
    def __init__(self, input_channels, output_channels):
        ...
        self.resbranch1x1 = BasicConv2d(input_channels, output_channels, kernel_size=1, stride=2)        # 修改为残差连接的支路，使用1*1卷积来建立残差连接，对整个Inception模块
    ...
    def forward(self, x):
        ...
        return torch.cat(output, 1)+resbranch1x1_output
...
class SEDenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        self.Inception_c = self._generate_inception_module(3, 64, 1, InceptionA_1_5_res)
        ...
        self.features = self._generate_inception_module(64, 64, 1, InceptionA_1_5_res)
