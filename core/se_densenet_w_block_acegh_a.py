...
def se_densenet121(pretrained=False, is_strict=False, **kwargs):
    ...
    """
    model = SEDenseNet(num_init_features=64, growth_rate=128, block_config=(6, 12, 24, 16),
                     **kwargs)
    if pretrained:
...
class SEDenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=128, block_config=(6, 12, 24, 16),
                 num_init_features=64, bn_size=4, drop_rate=0, num_classes=1000):       #经过计算，为了残差连接能实现，把这里的bn_size改为1合适，通道数全部改为32了     r如果需要加大必须要保证参数同步增加倍数
