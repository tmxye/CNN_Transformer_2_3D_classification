...
class InceptionA_1_1_res(nn.Module):
    def __init__(self, input_channels, output_channels):
        ...
        self.branch3x3stack = nn.Sequential(
            ...
            nn.Conv2d(16, int(output_channels*1), kernel_size=3, stride=1, padding=1),
        )
        self.branch3x3 = nn.Sequential(
            nn.Conv2d(input_channels, int(output_channels*1), kernel_size=3, stride=2, padding=1),
        )
        self.branch1x1 = nn.Sequential(
            nn.Conv2d(input_channels, int(output_channels*1), kernel_size=1, stride=2),        #这里是256     256-----10.6667     待会用这个补  8
        )
        self.resbranch1x1 = nn.Sequential(
            nn.Conv2d(input_channels, output_channels, kernel_size=1, stride=2, bias=False),
        )
        self.branchpool = nn.Sequential(
            # BasicConv2d(input_channels, int(output_channels*1), kernel_size=1),
            nn.Conv2d(input_channels, int(output_channels*1), kernel_size=1, stride=1, bias=False),
    ...
    def forward(self, x):
        ...
        InceptionA_1_5_res_out = self.bn2(torch.cat(output, 1))
...
class InceptionA_1_4_res(nn.Module):
    def __init__(self, input_channels, output_channels):
        ...
        self.branch3x3stack = nn.Sequential(
            ...
            nn.Conv2d(int(((input_channels*output_channels)**0.5)*0.125), int(output_channels*1), kernel_size=3, stride=1, padding=1),
        )
        self.branch3x3 = nn.Sequential(
            nn.Conv2d(input_channels, int(output_channels*1), kernel_size=3, stride=2, padding=1),
        )
