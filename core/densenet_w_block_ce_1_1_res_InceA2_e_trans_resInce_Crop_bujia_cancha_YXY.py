...
class InceptionA_1_1_res(nn.Module):
    ...
    def __init__(self, input_channels, output_channels):
        ...
        self.branchpool = nn.Sequential(
            nn.AvgPool2d(kernel_size=3, stride=2, padding=1),
            BasicConv2d_New(input_channels, int(output_channels*0.25), kernel_size=1)         #这里是256     256-----10.6667         待会用这个补    8
        )           #这里是 256        最后合起来是1536          现在合算起来是64-48=16     每个补8即可
...
class _TransitionLayer(nn.Sequential):
    def __init__(self, num_input_features, num_output_features):
        ...
        self.pool_Crop = nn.AvgPool2d(kernel_size=2, stride=2)
        self.conv_Crop = BasicConv2d(num_input_features*2, num_output_features, kernel_size=1, stride=1)
    ...
    def forward(self, x):
        ...
        out = self.conv_Crop(out)
        out = self.pool_Crop(out)
