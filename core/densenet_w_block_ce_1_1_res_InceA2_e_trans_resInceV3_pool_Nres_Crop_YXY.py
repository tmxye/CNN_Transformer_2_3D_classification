...
class InceptionC_res(nn.Module):
    ...
    def __init__(self, input_channels, output_channels):
        ...
        # 现在将步长改为2
        self.branch3x3stack = nn.Sequential(
            BasicConv2d(input_channels, int(input_channels*0.375), kernel_size=1),        #384---16
            BasicConv2d(int(input_channels*0.375), int(input_channels*0.4375), kernel_size=(1, 3), padding=(0, 1)),       # 448---18.666666667     给20算
            BasicConv2d(int(input_channels*0.4375), int(input_channels*0.5), kernel_size=(3, 1), padding=(1, 0)),     #512-----21.33333333       给24算
        )
        # self.branch3x3stacka = BasicConv2d(int(output_channels*0.5), int(output_channels*0.125), kernel_size=(1, 3), stride=2, padding=(0, 1))          #256----10.666667       给12算
        # self.branch3x3stackb = BasicConv2d(int(output_channels*0.5), int(output_channels*0.125), kernel_size=(3, 1), stride=2, padding=(1, 0))        #这里是 256+256     现在是12+12
        self.branch3x3stacka = nn.Sequential(
            BasicConv2d(int(input_channels*0.5), int(output_channels*0.125), kernel_size=(1, 3), stride=1, padding=(0, 1)),          #256----10.666667       给12算
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
        )
        self.branch3x3stackb = nn.Sequential(
            BasicConv2d(int(input_channels*0.5), int(output_channels*0.125), kernel_size=(3, 1), stride=1, padding=(1, 0)),        #这里是 256+256     现在是12+12
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
        )

        self.branch3x3 = BasicConv2d(input_channels, int(input_channels*0.375), kernel_size=1)
        # self.branch3x3a = BasicConv2d(int(output_channels*0.375), int(output_channels*0.125), kernel_size=(3, 1), stride=2, padding=(1, 0))          #256---10.667       给12算
        # self.branch3x3b = BasicConv2d(int(output_channels*0.375), int(output_channels*0.125), kernel_size=(1, 3), stride=2, padding=(0, 1))     #这里是256 +256      现在是12+12        目前是48
        self.branch3x3a = nn.Sequential(
            BasicConv2d(int(input_channels*0.375), int(output_channels*0.125), kernel_size=(3, 1), stride=1, padding=(1, 0)),          #256---10.667       给12算
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
        )
        self.branch3x3b = nn.Sequential(
            BasicConv2d(int(input_channels*0.375), int(output_channels*0.125), kernel_size=(1, 3), stride=1, padding=(0, 1)),     #这里是256 +256      现在是12+12        目前是48
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
        )

        # self.branch1x1 = BasicConv2d(input_channels, int(output_channels*0.25), kernel_size=1, stride=2)        #这里是256     256-----10.6667     待会用这个补  8
        self.branch1x1 = nn.Sequential(
            BasicConv2d(input_channels, int(output_channels*0.125), kernel_size=1, stride=1),        #这里是256     256-----10.6667     待会用这个补  8
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
        )
        ...
        self.branch3x3stack_A = nn.Sequential(
            # BasicConv2d(input_channels, int(output_channels*0.25), kernel_size=1),        #384---16
            # BasicConv2d(16, 20, kernel_size=(1, 3), padding=(0, 1)),       # 448---18.666666667     给20算
            # BasicConv2d(20, 16, kernel_size=(3, 1), padding=(1, 0)),     #512-----21.33333333       给24算
            # BasicConv2d(16, 20, kernel_size=(1, 3), padding=(0, 1)),       # 448---18.666666667     给20算
            ...
            # BasicConv2d(input_channels, int(output_channels*0.5), kernel_size=3, stride=1, padding=1),
            BasicConv2d(input_channels, int(((input_channels*output_channels)**0.5)*0.5), kernel_size=3, stride=1, padding=1),
            BasicConv2d(int(((input_channels*output_channels)**0.5)*0.5), int(output_channels*0.125), kernel_size=3, stride=1, padding=1),
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
        )
        ...
        self.branch3x3_A = nn.Sequential(
            ...
            # BasicConv2d(16, 20, kernel_size=(1, 3), padding=(0, 1)),       # 448---18.666666667     给20算
            # BasicConv2d(20, 16, kernel_size=(3, 1), stride=2, padding=(1, 0)),     #512-----21.33333333       给24算
            BasicConv2d(input_channels, int(output_channels*0.125), kernel_size=3, stride=1, padding=1),
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
            # BasicConv2d(16, 16, kernel_size=1, stride=2)
        )
    ...
    def forward(self, x):
        branch3x3stack_output_A = self.branch3x3stack_A(x)
        branch3x3_output_A = self.branch3x3_A(x)
        ...
        InceptionA_1_5_res_out = torch.cat(output, 1)+resbranch1x1_output
        del branch3x3stack_output
        del branch3x3_output_A
        del branch3x3_output
        del branch3x3stack_output_A
        del branch1x1_output
        del resbranch1x1_output
        del output
        del branchpool
...
class _TransitionLayer(nn.Sequential):
    def __init__(self, num_input_features, num_output_features):
        ...
        self.Inception_c = generate_inception_module(num_input_features*2, num_output_features, 1, InceptionC_res)
        # #   这个是1*1的过渡层
