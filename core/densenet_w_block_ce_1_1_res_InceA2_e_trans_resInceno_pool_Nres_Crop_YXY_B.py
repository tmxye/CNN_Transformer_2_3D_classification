...
class _TransitionLayer(nn.Sequential):
    def __init__(self, num_input_features, num_output_features):
        ...
        self.Inception_c = generate_inception_module(num_input_features, num_output_features, 1, origin)
        # #   这个是1*1的过渡层
...
class SEDenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        # self.Inception_c = self._generate_inception_module(3, 64, 1, FirstConv)
        # 这里也需要修改为 features
        self.features = self._generate_inception_module(3, 64, 1, FirstConv)
        ...
        # ]))
        #
        # # 由于第一个卷积层使用的是步长2的卷积和步长2的池化     这里暂时注销掉
