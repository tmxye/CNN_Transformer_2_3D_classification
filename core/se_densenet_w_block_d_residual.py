...
class _DenseLayer(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
        ...
        # # 这样修改之后，还不是原文的结构，相反上面的结构是本次我准备使用的结构    上面的结构四BN-Relu-Conv
        # # 下面开始对原文的结构进行复现，原文结构是没有BN的，残差连接在后面再进行修改
        # # self.add_module('norm1', nn.BatchNorm2d(num_input_features)),
        # # self.add_module('relu1', nn.ReLU(num_input_features, inplace=True)),
        # self.add_module('relu1', nn.ReLU(num_input_features)),
        # self.add_module('conv1', nn.Conv2d(num_input_features, bn_size *
        #                 growth_rate, kernel_size=1, stride=1, bias=False)),
        #  # 因此需要把SE放在1*1和3*3之后,文章的SE模块已经复现了
        # self.add_module("selayer", SELayer(channel=bn_size *  growth_rate)),
        #
        # # self.add_module('norm2', nn.BatchNorm2d(bn_size * growth_rate)),
        # # self.add_module('relu2', nn.ReLU(num_input_features, inplace=True)),
