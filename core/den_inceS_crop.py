...
class InceptionA_1_1_res(nn.Module):
    ...
    def __init__(self, input_channels, output_channels):
        ...
        self.branch3x3stack = nn.Sequential(
            ...
            BasicConv2d_New(input_channels, 16, kernel_size=3, stride=1, padding=1),
            BasicConv2d_New(16, int(output_channels*0.25), kernel_size=3, stride=1, padding=1),
...
class InceptionA_1_4_res(nn.Module):
    ...
    def __init__(self, input_channels, output_channels):
        ...
        print("int(((input_channels*output_channels)**0.5)", int((input_channels*output_channels)**0.5))
        # 现在将步长改为2
        self.branch3x3stack = nn.Sequential(
            # BasicConv2d(input_channels, int(output_channels*0.25), kernel_size=1),        #384---16
            # BasicConv2d(16, 20, kernel_size=(1, 3), padding=(0, 1)),       # 448---18.666666667     给20算
            ...
            # BasicConv2d(20, 16, kernel_size=(3, 1), padding=(1, 0)),     #512-----21.33333333       给24算
            # BasicConv2d(input_channels, int(output_channels*0.5), kernel_size=3, stride=1, padding=1),
            BasicConv2d_New(input_channels, int(((input_channels*output_channels)**0.5)*0.125), kernel_size=3, stride=1, padding=1),
            BasicConv2d_New(int(((input_channels*output_channels)**0.5)*0.125), int(output_channels*0.25), kernel_size=3, stride=1, padding=1),
            nn.MaxPool2d(kernel_size=3, stride=2, padding=1),
        )
...
class InceptionA_1_5_res(nn.Module):
    ...
    def __init__(self, input_channels, output_channels):
        ...
        self.branch3x3stack = nn.Sequential(
            ...
            # BasicConv2d(20, 16, kernel_size=(3, 1), padding=(1, 0)),     #512-----21.33333333       给24算
            # BasicConv2d(input_channels, int(output_channels*0.5), kernel_size=3, stride=1, padding=1),
            BasicConv2d(input_channels, int(((input_channels*output_channels)**0.5)*0.125), kernel_size=3, stride=1, padding=1),
            BasicConv2d(int(((input_channels*output_channels)**0.5)*0.125), int(output_channels*0.25), kernel_size=3, stride=1, padding=1),
...
class _TransitionLayer(nn.Sequential):
    ...
    def forward(self, x, firstbatchsize=16):
        ...
        out = torch.cat([x.cuda(), out.cuda()], 1)
