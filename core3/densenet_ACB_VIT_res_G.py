...
class SEDenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        self._DenseBlockA = _DenseBlockA(num_input_features=num_features, growth_rate=growth_rate, bn_size=bn_size,
                                         drop_rate=drop_rate)
        num_features = num_features + 6 * growth_rate
        # num_features = 256
        self.resA = nn.Conv2d(num_init_features, num_features, kernel_size=1, stride=1, groups=num_init_features//4, bias=False)
        self._TransitionA = _Transition(num_input_features=num_features, num_output_features=num_features // 2)
        num_features = num_features // 2

        num_featuresB = num_features
        self._DenseBlockB = _DenseBlockB(num_input_features=num_features, growth_rate=growth_rate, bn_size=bn_size,
                                         drop_rate=drop_rate)
        num_features = num_features + 12 * growth_rate
        # num_features = 512
        self.resB = nn.Conv2d(num_featuresB, num_features, kernel_size=1, stride=1, groups=num_featuresB//4, bias=False)
        self._TransitionB = _Transition(num_input_features=num_features, num_output_features=num_features // 2)
        num_features = num_features // 2

        num_featuresC = num_features
        self._DenseBlockC = _DenseBlockC(num_input_features=num_features, growth_rate=growth_rate, bn_size=bn_size,
                                         drop_rate=drop_rate)
        num_features = num_features + 24 * growth_rate
        # num_features = 1024
        self.resC = nn.Conv2d(num_featuresC, num_features, kernel_size=1, stride=1, groups=num_featuresC//4, bias=False)
        self._TransitionC = _Transition(num_input_features=num_features, num_output_features=num_features // 2)
        num_features = num_features // 2

        num_featuresD = num_features
        self._DenseBlockD = _DenseBlockD(num_input_features=num_features, growth_rate=growth_rate, bn_size=bn_size,
                                         drop_rate=drop_rate)
        num_features = num_features + 16 * growth_rate
        # num_features = 1024
        self.resD = nn.Conv2d(num_featuresD, num_features, kernel_size=1, stride=1, bias=False)
