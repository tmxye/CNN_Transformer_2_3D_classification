...
class _DenseLayer(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
        ...
        self.conv1 = nn.Conv2d(num_input_features, bn_size * growth_rate, kernel_size=1, stride=1, groups=growth_rate//4, bias=False)
        ...
        self.scales = 2
        self.conv_1 = ACBlock(bn_size * growth_rate //2, growth_rate//2, kernel_size=3, stride=1,  padding=1, dilation=1, groups= 16,
                              padding_mode='zeros', deploy=False, gamma_init=1/3)
        self.conv3 = ACBlock(bn_size * growth_rate, growth_rate, kernel_size=3, stride=1,  padding=1, dilation=1, groups= 32,
                              padding_mode='zeros', deploy=False, gamma_init=1/3)
        self.conv_2 = ACBlock(bn_size * growth_rate //2, growth_rate//2, kernel_size=3, stride=1,  padding=1, dilation=1, groups= 16,
                              padding_mode='zeros', deploy=False, gamma_init=1/3)
        # self.conv_2 = ACBlock(bn_size * growth_rate //2, growth_rate//2, kernel_size=5, stride=1,  padding=2, dilation=1, groups= 16,
        #                       padding_mode='zeros', deploy=False, gamma_init=1/3)
        self.conv_3 = ACBlock(growth_rate//2, growth_rate//2, kernel_size=3, stride=1,  padding=1, dilation=1, groups= 16,
                              padding_mode='zeros', deploy=False, gamma_init=1/3)
        # self.SElayer = SELayer(channel=32)
    ...
    def forward(self, x):
        ...
        # conv_1 = self.conv_1(xs[0])
        # conv_2 = self.conv_2(xs[1])
        # conv_2 = self.conv_3(conv_2)
        # new_features = torch.cat([conv_1, conv_2], 1)
        # new_features = self.SElayer(new_features)
        if self.drop_rate > 0:
