...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 drop_path=0., act_layer=nn.ReLU, norm_layer=partial(nn.LayerNorm, eps=1e-6), vis=False):
        super().__init__()
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim//32, act_layer=act_layer, drop=drop)
...
class _DenseLayerA(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
        ...
        self.bn1 = nn.BatchNorm2d(num_input_features)
        self.relu1 = nn.ReLU(inplace=True)
        self.conv1 = nn.Conv2d(num_input_features, int(growth_rate * 0.875), kernel_size=1, stride=1, bias=False)
        self.conv1A = nn.Conv2d(num_input_features, growth_rate-int(growth_rate * 0.875), kernel_size=1, stride=1, bias=False)
        self.bn3 = nn.BatchNorm2d(int(growth_rate * 0.875))
        self.bn3A = nn.BatchNorm2d(growth_rate-int(growth_rate * 0.875))
        self.norm = nn.LayerNorm(growth_rate-int(growth_rate * 0.875))
        self.relu3 = nn.ReLU(inplace=True)
        self.conv3 = nn.Conv2d(int(growth_rate * 0.875), int(growth_rate * 0.875), kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, x):
        new_features = self.bn1(x)
        new_features = self.relu1(new_features)
        x_A = self.conv1(new_features)
        x_B = self.conv1A(new_features)
        x_A = self.bn3(x_A)
        x_A = self.relu3(x_A)
        # x_B = self.bn3A(x_B)
        # x_B = self.norm(x_B)
        x_B = self.relu3(x_B)
        # x_A, x_B = new_features.split([int(new_features.shape[1] * 0.875), int(new_features.shape[1]) - int(new_features.shape[1] * 0.875)], dim=1)
        x_A = self.conv3(x_A)
...
class _DenseLayerB(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
        ...
        self.bn1 = nn.BatchNorm2d(num_input_features)
        self.relu1 = nn.ReLU(inplace=True)
        self.conv1 = nn.Conv2d(num_input_features, int(growth_rate * 0.75), kernel_size=1, stride=1, bias=False)
        self.conv1A = nn.Conv2d(num_input_features, growth_rate-int(growth_rate * 0.75), kernel_size=1, stride=1, bias=False)
        self.bn3 = nn.BatchNorm2d(int(growth_rate * 0.75))
        self.bn3A = nn.BatchNorm2d(growth_rate-int(growth_rate * 0.75))
        self.norm = nn.LayerNorm(growth_rate-int(growth_rate * 0.75))
        self.relu3 = nn.ReLU(inplace=True)
        self.conv3 = nn.Conv2d(int(growth_rate * 0.75), int(growth_rate * 0.75), kernel_size=3, stride=1, padding=1, bias=False)
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
...
class _DenseLayerC(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
        ...
        self.bn1 = nn.BatchNorm2d(num_input_features)
        self.relu1 = nn.ReLU(inplace=True)
        self.conv1 = nn.Conv2d(num_input_features, int(growth_rate * 0.5), kernel_size=1, stride=1, bias=False)
        self.conv1A = nn.Conv2d(num_input_features, growth_rate-int(growth_rate * 0.5), kernel_size=1, stride=1, bias=False)
        self.bn3 = nn.BatchNorm2d(int(growth_rate * 0.5))
        self.bn3A = nn.BatchNorm2d(growth_rate-int(growth_rate * 0.5))
        self.relu3 = nn.ReLU(inplace=True)
        self.conv3 = nn.Conv2d(int(growth_rate * 0.5), int(growth_rate * 0.5), kernel_size=3, stride=1, padding=1, bias=False)
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
...
class _DenseLayerD(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
        ...
        self.bn1 = nn.BatchNorm2d(num_input_features)
        self.relu1 = nn.ReLU(inplace=True)
        self.conv1 = nn.Conv2d(num_input_features, int(growth_rate * 0.25), kernel_size=1, stride=1, bias=False)
        self.conv1A = nn.Conv2d(num_input_features, growth_rate-int(growth_rate * 0.25), kernel_size=1, stride=1, bias=False)
        self.bn3 = nn.BatchNorm2d(int(growth_rate * 0.25))
        self.bn3A = nn.BatchNorm2d(growth_rate-int(growth_rate * 0.25))
        self.relu3 = nn.ReLU(inplace=True)
        self.conv3 = nn.Conv2d(int(growth_rate * 0.25), int(growth_rate * 0.25), kernel_size=3, stride=1, padding=1, bias=False)
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)
        self.trans = Block(dim=7*7, num_heads=7, mlp_ratio=4, qkv_bias=True, qk_scale=None,
            drop=0., attn_drop=0., drop_path=0., norm_layer=partial(nn.LayerNorm, eps=1e-6),)
        # self.trans = MHSA(growth_rate, 28, 28)
