...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=7, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.qkv = nn.Linear(dim, dim * 3 //7, bias=qkv_bias)
    ...
    def forward(self, x):
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads //7).permute(2, 0, 3, 1, 4)
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, N, C //7)
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
