...
class _DenseLayer(nn.Sequential):
    ...
    def forward(self, x):
        ...
        #### NAM
        residual = new_features
        new_features = self.bn2(new_features)
        weight_bn = self.bn2.weight.data.abs() / torch.sum(self.bn2.weight.data.abs())
        new_features = new_features.permute(0, 2, 3, 1).contiguous()
        new_features = torch.mul(weight_bn, new_features)
        new_features = new_features.permute(0, 3, 1, 2).contiguous()
        new_features = torch.sigmoid(new_features) * residual
