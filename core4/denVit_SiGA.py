...
class _DenseLayer(nn.Sequential):
    ...
    def forward(self, x):
        ...
        x_permute = new_features.permute(0, 2, 3, 1).view(b, -1, c)
        x_att_permute = self.channel_attention(x_permute).view(b, h, w, c)
        x_channel_att = x_att_permute.permute(0, 3, 1, 2)
        new_features = new_features * x_channel_att
        x_spatial_att = self.spatial_attention(new_features).sigmoid()
        new_features = new_features * x_spatial_att


        b, c, h, w = new_features.size()
        n = w * h - 1
        x_minus_mu_square = (new_features - new_features.mean(dim=[2, 3], keepdim=True)).pow(2)
        y = x_minus_mu_square / (4 * (x_minus_mu_square.sum(dim=[2, 3], keepdim=True) / n + self.e_lambda)) + 0.5
        new_features = new_features * self.activaton(y)
