...
class _DenseLayerA(nn.Sequential):
    ...
    def forward(self, x):
        ...
        new_features = self.bn3(new_features)
        new_features = self.relu3(new_features)
        x_A, x_B = new_features.split([int(new_features.shape[1] * 0.875), int(new_features.shape[1] - new_features.shape[1] * 0.875)], dim=1)
        x_A = self.conv3(x_A)
...
class _DenseLayerB(nn.Sequential):
    ...
    def forward(self, x):
        ...
        new_features = self.bn3(new_features)
        new_features = self.relu3(new_features)
        x_A, x_B = new_features.split([int(new_features.shape[1] * 0.75), int(new_features.shape[1]) - int(new_features.shape[1] * 0.75)], dim=1)
        x_A = self.conv3(x_A)
...
class _DenseLayerC(nn.Sequential):
    ...
    def forward(self, x):
        ...
        new_features = self.bn3(new_features)
        new_features = self.relu3(new_features)
        x_A, x_B = new_features.split([int(new_features.shape[1] * 0.5), int(new_features.shape[1]) - int(new_features.shape[1] * 0.5)], dim=1)
        x_A = self.conv3(x_A)
...
class _DenseLayerD(nn.Sequential):
    ...
    def forward(self, x):
        ...
        new_features = self.bn3(new_features)
        new_features = self.relu3(new_features)
        x_A, x_B = new_features.split([int(new_features.shape[1] * 0.25), int(new_features.shape[1]) - int(new_features.shape[1] * 0.25)], dim=1)
        x_A = self.conv3(x_A)
        B, C, W, H  = x_B.shape
        x_B = x_B.flatten(2)
        x_B = self.trans(x_B).reshape(B, C, W, H)
