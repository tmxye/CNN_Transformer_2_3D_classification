...
class SEDenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        # filtersA = [64, 128, 256]       # 这个是原始的
        filtersA = [256, 512, 1024]
    ...
    def forward(self, x):
        ...
        # inputsSkip1 = inputsSkip1.to('cuda:0')
        # DenseBlockA = DenseBlockA.to('cuda:0')      #256 维度
        #  input, gating_signal     input指的是输入
        AttentionA, att1 = self.attentionblock1(inputsSkip1, DenseBlockA)
        # AttentionA = AttentionA.to('cuda:0')
        DenseBlockAYXY = torch.cat([AttentionA, DenseBlockA], 1)
        TransitionA = self._TransitionA(DenseBlockAYXY)
        del att1, AttentionA, DenseBlockAYXY
        torch.cuda.empty_cache()

        DenseBlockB = self._DenseBlockB(TransitionA)
        inputsSkip2 = self.maxpool(inputsSkip1)      #64 维度
        DenseBlockA = self.maxpool(DenseBlockA)      #256 维度
        DenseBlockA = self.Skip2(DenseBlockA)      #64 维度
        inputsSkip2 = torch.cat([inputsSkip2, DenseBlockA], 1)      #128
        # inputsSkip2 = inputsSkip2.to('cuda:0')
        # DenseBlockB = DenseBlockB.to('cuda:0')
        AttentionB, att2 = self.attentionblock2(inputsSkip2, DenseBlockB)      # AttentionB和DenseBlockB是一样维度
        # AttentionB = AttentionB.to('cuda:0')
        DenseBlockBYXY = torch.cat([AttentionB, DenseBlockB], 1)
        TransitionB = self._TransitionB(DenseBlockBYXY)
        del inputsSkip1, att2, AttentionB, DenseBlockBYXY
        torch.cuda.empty_cache()
        ...
        DenseBlockB = self.Skip3(DenseBlockB)      #128 维度
        inputsSkip3 = torch.cat([inputsSkip3, DenseBlockB], 1)      #256
        # inputsSkip3 = inputsSkip3.to('cuda:0')
        # DenseBlockC = DenseBlockC.to('cuda:0')
        AttentionC, att3 = self.attentionblock3(inputsSkip3, DenseBlockC)      # AttentionB和DenseBlockB是一样维度
        # AttentionC = AttentionC.to('cuda:0')
