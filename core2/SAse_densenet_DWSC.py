...
from core2.nonlocal_layer import NONLocalBlock2D
...
class SeparableConv2d(nn.Module):
    ...
    # def __init__(self, in_channels, out_channels, weight, kernel_size=1, stride=1, padding=0, dilation=1, bias=False):
...
class _DenseLayer(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
        ...
        self.conv1 = nn.Conv2d(num_input_features, bn_size * growth_rate, kernel_size=1, stride=1,bias=False)
        # self.add_module('conv1', SeparableConv2d(num_input_features, bn_size * growth_rate, kernel_size=1, stride=1, bias=False)),
        ...
        self.conv3 = SeparableConv2d(bn_size * growth_rate, growth_rate, kernel_size=3, stride=1, padding=1, bias=False)
        # self.add_module('conv2', nn.Conv2d(bn_size * growth_rate, growth_rate, kernel_size=3, stride=1, padding=1, bias=False)),
        # self.add_module('conv2', SeparableConv2d(bn_size * growth_rate, growth_rate, kernel_size=3, stride=1, padding=1, bias=False)),
...
class _Transition(nn.Sequential):
    def __init__(self, num_input_features, num_output_features):
        ...
        # # self.add_module = NONLocalBlock2D(in_channels=num_input_features, inter_channels=filters[4] // 4)
        # self.features.add_module("NONLocalBlock2D_%db" % (i + 1), NONLocalBlock2D(in_channels=num_features, inter_channels=num_features // 4))
        # self.SA = NONLocalBlock2D(in_channels=num_input_features, inter_channels=num_input_features // 4)
        # by YXY这里过渡层本来是不应该使用SE的,但是由于在密集单元内使用空间注意力会导致参数爆炸增长,在这里添加空间注意力和SE
        # self.features.add_module("SELayer_%db" % (i + 1), SELayer(channel=num_features))
        self.SE = SELayer(channel=num_input_features)
...
class SEDenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
        ...
        #     ('conv0', nn.Conv2d(3, num_init_features, kernel_size=7, stride=2, padding=3, bias=False)),
            ('conv0', SeparableConv2d(3, num_init_features, kernel_size=7, stride=2, padding=3, bias=False)),
