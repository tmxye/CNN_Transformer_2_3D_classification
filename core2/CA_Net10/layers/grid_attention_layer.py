...
from core2.CA_Net.networks_other import init_weights

class _GridAttentionBlockND1(nn.Module):
    def __init__(self, in_channels, gating_channels, inter_channels=None, dimension=3, mode='concatenation',
                 sub_sample_factor=(2,2,2)):
        super(_GridAttentionBlockND1, self).__init__()
        ...
        self.phi2 = conv_nd(in_channels=self.gating_channels*2, out_channels=self.inter_channels,
                           kernel_size=(1, 1), stride=1, padding=0, bias=True)
        self.psi = conv_nd(in_channels=self.inter_channels*2, out_channels=1, kernel_size=1, stride=1, padding=0, bias=True)
    ...
    def _concatenation(self, x, g):
        ...
        f = F.relu((torch.cat([theta_x, phi_g], dim=1)), inplace=True)
...
class GridAttentionBlock2D1(_GridAttentionBlockND1):
    def __init__(self, in_channels, gating_channels, inter_channels=None, mode='concatenation',
                 sub_sample_factor=(2, 2)):
        super(GridAttentionBlock2D1, self).__init__(in_channels,
                                                   inter_channels=inter_channels,
                                                   gating_channels=gating_channels,
                                                   dimension=2, mode=mode,
                                                   sub_sample_factor=sub_sample_factor,
...
class MultiAttentionBlock(nn.Module):
    def __init__(self, in_size, gate_size, inter_size, nonlocal_mode, sub_sample_factor):
        ...
        self.gate_block_1 = GridAttentionBlock2D(in_channels=in_size, gating_channels=gate_size,
                                                 ...
                                                 sub_sample_factor=sub_sample_factor)
        self.gate_block_2 = GridAttentionBlock2D1(in_channels=in_size, gating_channels=gate_size,
                                                 inter_channels=inter_size, mode=nonlocal_mode,
                                                 sub_sample_factor=sub_sample_factor)
    ...
    def forward(self, input, gating_signal):
        ...
        # gating_signal = gating_signal.to('cuda:0')
        gate_1, attention_1 = self.gate_block_1(input, gating_signal)
        gate_2, attention_2 = self.gate_block_2(input, gating_signal)
        # gate_2, attention_2 = self.gate_block_2(gating_signal, input)

        # return self.combine_gates(torch.cat([gate_1, gate_2], 1)), torch.cat([attention_1, attention_2], 1)
        gate_1A = gate_1.cuda()
        gate_2A = gate_2.cuda()
        attention_1A = attention_1.to('cuda:0')
        attention_2A = attention_2.to('cuda:0')
        # print("gata_1", (gate_1A*gate_1A).shape)
        # print("gata_2", gate_2A.shape)
        # gate_1 = self.combine_gates(torch.cat([gate_1A, gate_2A], 1))
        gate_1 = torch.cat([gate_1A, gate_2A], 1)

        x = gate_1
        m_batchsize, _, height, width = gate_1A.size()
        proj_query = self.query_conv(gate_1A)
        proj_query_H = proj_query.permute(0,3,1,2).contiguous().view(m_batchsize*width,-1,height).permute(0, 2, 1)
        proj_query_W = proj_query.permute(0,2,1,3).contiguous().view(m_batchsize*height,-1,width).permute(0, 2, 1)
        # proj_queryY = self.query_conv(proj_query)
        # proj_query_HY = proj_queryY.permute(0,3,1,2).contiguous().view(m_batchsize*width,-1,height).permute(0, 2, 1)
        # proj_query_WY = proj_queryY.permute(0,2,1,3).contiguous().view(m_batchsize*height,-1,width).permute(0, 2, 1)
        proj_key = self.key_conv(gate_2A)
        proj_key_H = proj_key.permute(0,3,1,2).contiguous().view(m_batchsize*width,-1,height)
        proj_key_W = proj_key.permute(0,2,1,3).contiguous().view(m_batchsize*height,-1,width)
        # proj_keyY = self.key_conv(proj_key)
        # proj_key_HY = proj_keyY.permute(0,3,1,2).contiguous().view(m_batchsize*width,-1,height)
        # proj_key_WY = proj_keyY.permute(0,2,1,3).contiguous().view(m_batchsize*height,-1,width)
        proj_value = self.value_conv(x)
        proj_value_H = proj_value.permute(0,3,1,2).contiguous().view(m_batchsize*width,-1,height)
        proj_value_W = proj_value.permute(0,2,1,3).contiguous().view(m_batchsize*height,-1,width)
        energy_H = (torch.bmm(proj_query_H, proj_key_H)+self.INF(m_batchsize, height, width)).view(m_batchsize,width,height,height).permute(0,2,1,3)
        energy_W = torch.bmm(proj_query_W, proj_key_W).view(m_batchsize,height,width,width) # https://copyfuture.com/blogs-details/202203162208586247
        ...
        x = self.gamma * (out_H + out_W) + proj_value
        # print("x, ", x.shape)

        # 第二个CCNet      https://blog.csdn.net/qq_36321330/article/details/105460926
        # print("gate_1", x.shape)
        # print("gate_1", self.in_dim)
        # gate_1 = torch.split(x, self.in_dim//2, dim=1)
        # gate_1A, gate_2A = gate_1[0], gate_1[1]
        # print("gate_1A", gate_1A.shape)
        gate_1A = x
        gate_2A = x
        m_batchsize, _, height, width = gate_1A.size()
        proj_query = self.query_conv(gate_1A)
        ...
        # gata_1 = self.combine_gates(torch.cat([gate_1, gate_2], 1))

        gata_2 = torch.cat([attention_1A, attention_2A], 1)
        return gata_1, gata_2




# 这个是可以进行输入多倍特征图的，方便密集连接        暂时设置为2
class MultiAttentionBlockYXY(nn.Module):
    def __init__(self, in_size, gate_size, inter_size, nonlocal_mode, sub_sample_factor):
        super(MultiAttentionBlockYXY, self).__init__()
        self.gate_block_1 = GridAttentionBlock2D(in_channels=in_size, gating_channels=gate_size,
                                                 inter_channels=inter_size, mode=nonlocal_mode,
                                                 sub_sample_factor=sub_sample_factor)
        self.gate_block_2 = GridAttentionBlock2D(in_channels=in_size, gating_channels=gate_size,
                                                 inter_channels=inter_size, mode=nonlocal_mode,
                                                 sub_sample_factor=sub_sample_factor)
        self.combine_gates = nn.Sequential(nn.Conv2d(in_size*2, in_size //2, kernel_size=1, stride=1, padding=0),
    ...
    def forward(self, input, gating_signal):
        ...
        attention_1A = attention_1.to('cuda:0')
        attention_2A = attention_2.to('cuda:0')
        gata_1 = self.combine_gates(torch.cat([gate_1A, gate_2A], 1))
        # gata_1 = self.combine_gates(torch.cat([gate_1, gate_2], 1))
        gata_2 = torch.cat([attention_1A, attention_2A], 1)
        return gata_1, gata_2
