...
from .layer_CAM import MultiSpectralAttentionLayer
...
class SECAM(nn.Module):
    ...
    def forward(self, x):
        ...
        y = self.avg_pool(x)            # torch.Size([1, 64, 1, 1])
        # print("y", y.shape)
        # y = self.fc(y).view(b, c, 1, 1)
        y = self.fc(y)     # ([1, 64, 1, 1])
        # print("y", y.shape)
        return x * y
...
class _DenseLayer(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
        ...
        # self.SElayer = SECAM(channel=num_input_features)
...
class _DenseLayerB(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):
        ...
        self.SElayer = CoordAtt(num_input_features, num_input_features)
        self.bn1 = nn.BatchNorm2d(num_input_features)
        self.relu1 = nn.ReLU(inplace=True)
        self.conv1 = nn.Conv2d(num_input_features, bn_size * growth_rate, kernel_size=1, stride=1,bias=False)
        self.bn3 = nn.BatchNorm2d(bn_size * growth_rate)
