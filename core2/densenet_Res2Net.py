...
#             [conv3x3(bn_size * growth_rate // self.scales, growth_rate // self.scales, stride=1, groups=4) for _ in range(self.scales - 1)])  # k1,k2,k3后接3×3卷积，分组卷积
#         norm_layer = nn.BatchNorm2d
...
#         #         ys.append(xs[s])
#         #         print("A")
#         #         print(xs[s].size())     # torch.Size([1, 64, 56, 56])
#         #     elif s == 1:
#         #         ys.append(self.relu(self.bn2[s-1](self.conv2[s-1](xs[s]))))
#         #         print("B")
#         #         print(self.conv2[s-1](xs[s]).size())        #v torch.Size([1, 16, 56, 56])
#         #     else:
#         #         ys.append(self.relu(self.bn2[s-1](self.conv2[s-1](xs[s] + ys[-1]))))
#         # new_features = torch.cat(ys, 1)#concat操作
#         # # print(new_features.size())      #  running_mean should contain 144 elements not 96  torch.Size([1, 80, 56, 56])
#         # # new_features += x     # 残差连接再考虑
#
#         # 由于密集连接也会直接将输入传输到后续层
#
#         # print("xs_new_features", new_features.size())  # xs_new_features torch.Size([1, 128, 56, 56])
#         xs = torch.chunk(new_features, self.scales, 1)#将输入张量进行分块
#         # print("xs[0A]", xs[0].size())  # xs[0A] torch.Size([1, 64, 56, 56])
#         ys = []
#         for s in range(self.scales):
#             if s == 0:
#                 # print("xs[0]", xs[s].size())        # xs[0] torch.Size([1, 64, 56, 56])
#                 ys0 = self.relu(self.bn2[s](self.conv2[s](xs[s])))
#                 # print("ys0", ys0.size())            # ys0 torch.Size([1, 16, 56, 56])
#             elif s == 1:
#                 print("xs[1]", xs[s].size())        # xs[1] torch.Size([1, 64, 56, 56])
#                 ys1 = self.relu(self.bn2[s-1](self.conv2[s-1](xs[s])))
#                 print("B")
#                 print(self.conv2[s-1](xs[s]).size())        #v torch.Size([1, 16, 56, 56])
#             else:
#                 ys.append(self.relu(self.bn2[s-1](self.conv2[s-1](xs[s] + ys[-1]))))
#                 print("C")
#                 print(self.bn2[s-1](self.conv2[s-1](xs[s] + ys[-1])).size())        #v torch.Size([1, 16, 56, 56])
...
#
# #       改进为Res2Net。为了让输入和中间特征图可以相加，将bn_size改为1，这里直接删除
# class _DenseLayer(nn.Sequential):
...
#         self.scales = 4
#         self.conv2 = nn.ModuleList(
#             [conv3x3(growth_rate // self.scales, growth_rate // self.scales, stride=1, groups=4) for _ in range(self.scales - 1)])  # k1,k2,k3后接3×3卷积，分组卷积
#         norm_layer = nn.BatchNorm2d
#         self.bn2 = nn.ModuleList([norm_layer(growth_rate // self.scales) for _ in range(self.scales-1)])
#         self.relu = nn.ReLU(inplace=True)
#         self.drop_rate = drop_rate
...
#                 ys0 = self.relu(self.bn2[s](self.conv2[s](xs[s])))
#             elif s == 1:
#                 ys1 = self.relu(self.bn2[s-1](self.conv2[s-1](xs[s] + ys0)))
#             elif s == 2:
#                 ys2 = self.relu(self.bn2[s-1](self.conv2[s-1](xs[s] + ys1)))
#             else:
#                 ys3 = self.relu(self.bn2[s-1](self.conv2[s-1](xs[s] + ys2)))
#                 # ys.append(self.relu(self.bn2[s-1](self.conv2[s-1](xs[s] + ys[-1]))))
#         # new_features = torch.cat([ys0, ys1], 1)#concat操作
