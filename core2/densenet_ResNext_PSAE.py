...
class PSAModule(nn.Module):
    ...
    def __init__(self, inplans, planes, conv_kernels=[1, 3], stride=1, conv_groups=[4, 8]):
...
class _DenseLayer(nn.Sequential):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate):        # 参数量Params size (MB): 13.84
        ...
        self.conv1 = nn.Conv2d(num_input_features, bn_size * growth_rate, kernel_size=1, stride=1, padding=0, groups=4, bias=False)
        self.bnA = nn.BatchNorm2d(num_input_features)
        self.convA = nn.Conv2d(num_input_features, growth_rate, kernel_size=1, stride=1, padding=0, groups=4, bias=False)
        # self.convA = nn.Conv2d(num_input_features, growth_rate, kernel_size=1, stride=1, padding=0, bias=False)
        ...
        self.conv2 = nn.ModuleList(
            [PSAModule(bn_size * growth_rate // self.scales, bn_size * growth_rate // self.scales, stride=1) for _ in range(self.scales - 1)])
        # self.conv2 = nn.ModuleList(
    ...
    def forward(self, x):
        ...
        new_features1 = self.bnA(x)
        new_features1 = self.convA(new_features1)
