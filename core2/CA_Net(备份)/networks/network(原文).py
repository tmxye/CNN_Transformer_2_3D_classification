...
from core2.CA_Net.layers.grid_attention_layer import GridAttentionBlock2D, MultiAttentionBlock
...
class Comprehensive_Atten_Unet(nn.Module):
    ...
    #              nonlocal_mode='concatenation', attention_dsample=(1, 1)):
        ...
        self.attentionblock1 = MultiAttentionBlock(in_size=filters[0], gate_size=filtersA[2], inter_size=filters[0],
                                                   nonlocal_mode=nonlocal_mode, sub_sample_factor=attention_dsample)
        self.attentionblock2 = MultiAttentionBlock(in_size=filters[1], gate_size=filters[2], inter_size=filters[1],
                                                   nonlocal_mode=nonlocal_mode, sub_sample_factor=attention_dsample)
        self.attentionblock3 = MultiAttentionBlock(in_size=filters[2], gate_size=filters[3], inter_size=filters[2],
    ...
    # #     self.l1 = nn.Sequential(*list(model.children())[:-1]).to('cuda:1')
    #     self.l1 = nn.Sequential(*list(self.children())[:-1]).to('cuda:1')
    #     self.last = list(self.children())[-1]

    # # #############################by YXY ,实现募集连接  ################################
    #     self.Skip1 = conv_blockYXY(self.in_channels, filtersA[1])
    ...
    def forward(self, inputs):
        ...
        # conv1 = conv1.to('cuda:1')
        maxpool1 = self.maxpool1(conv1)
        # maxpool1 = maxpool1.to('cuda:1')

        conv2 = self.conv2(maxpool1)
        # conv2 = conv2.to('cuda:1')
        maxpool2 = self.maxpool2(conv2)
        # maxpool2 = maxpool2.to('cuda:1')

        conv3 = self.conv3(maxpool2)
        # conv3 = conv3.to('cuda:1')
        maxpool3 = self.maxpool3(conv3)
        # maxpool3 = maxpool3.to('cuda:1')

        conv4 = self.conv4(maxpool3)
        # conv4 = conv4.to('cuda:1')
        maxpool4 = self.maxpool4(conv4)
        # maxpool4 = maxpool4.to('cuda:1')

        # Gating Signal Generation
        center = self.center(maxpool4)
        # center = center.to('cuda:1')
    ...
    #     inputsSkip1 = torch.cat([inputsSkip, conv1], 1)
        ...
        conv1A = conv1.to('cuda:1')
        # conv1A = inputsSkip1.to('cuda:1')
        up2A = up2.to('cuda:1')
