...
class PSAModule(nn.Module):
    ...
    def __init__(self, inplans, planes, conv_kernels=[3, 5, 7, 9], stride=1, conv_groups=[4, 4, 8, 8]):
    # def __init__(self, inplans, planes, conv_kernels=[3, 5, 7, 9], stride=1, conv_groups=[1, 2, 4, 4]):     # 参数量为Params size (MB): 61.30
    # def __init__(self, inplans, planes, conv_kernels=[3, 5, 7, 9], stride=1, conv_groups=[1, 2, 4, 8]):     # 参数量为Params size (MB): 56.71
        ...
        self.se = SEWeightModule(channels=planes // 4)
        # self.avg_pool = nn.AdaptiveAvgPool2d(1)
        # self.fc1 = nn.Conv2d(planes // 4, planes // 4 //16, kernel_size=1, padding=0)
        # self.relu = nn.ReLU(inplace=True)
        # self.fc2 = nn.Conv2d(planes // 4 //16, planes // 4, kernel_size=1, padding=0)
        # self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        #
        # out = self.avg_pool(x1)
        # out = self.fc1(out)
        # out = self.relu(out)
        # out = self.fc2(out)
        # x1_se = self.sigmoid(out)
        #
        # out = self.avg_pool(x2)
        # out = self.fc1(out)
        # out = self.relu(out)
        # out = self.fc2(out)
        # x2_se = self.sigmoid(out)
        #
        # out = self.avg_pool(x3)
        # out = self.fc1(out)
        # out = self.relu(out)
        # out = self.fc2(out)
        # x3_se = self.sigmoid(out)
        #
        # out = self.avg_pool(x4)
        # out = self.fc1(out)
        # out = self.relu(out)
        # out = self.fc2(out)
        # x4_se = self.sigmoid(out)
        ...
        attention_vectors = x_se.view(batch_size, 4, self.split_channel, x_se.shape[2], x_se.shape[3])
