...
class MyDataset(torch.utils.data.Dataset):  # 创建自己的类：MyDataset,这个类是继承的torch.utils.data.Dataset
    ...
    def __getitem__(self, index):  # 这个方法是必须要有的，用于按照索引读取每个元素的具体内容
        ...
        imgA = self.transform(img)  # 是否进行transform

        fnend = fn.split('/')  # 采用"."进行分割       \\这样获取到最后的，0.jpg   465.jpg
        fnint = fnend[-1].split('.')  # 采用"."进行分割       \\这样可以获取到第一个465     int(fnint[0])
        index = int(fnint[0])           # 0
        # https://www.python51.com/jc/11697.html
        fnB = fn.replace("{}.jpg".format(index), "{}.jpg".format(index+1))
        img = Image.open(fnB).convert('RGB')  # 按照path读入图片from PIL import Image # 按照路径读取图片
        imgB = self.transform(img)  # 是否进行transform
        fnC = fn.replace("{}.jpg".format(index), "{}.jpg".format(index+2))
        img = Image.open(fnC).convert('RGB')  # 按照path读入图片from PIL import Image # 按照路径读取图片
        imgC = self.transform(img)  # 是否进行transform
        fnD = fn.replace("{}.jpg".format(index), "{}.jpg".format(index+3))
        img = Image.open(fnD).convert('RGB')  # 按照path读入图片from PIL import Image # 按照路径读取图片
        imgD = self.transform(img)  # 是否进行transform
        
        # ([4, 3, 224, 224, 32])        这里的图片是3, 224, 224       因此加在第一个维度
        imgA = np.expand_dims(imgA, 1)
        imgB = np.expand_dims(imgB, 1)
        imgC = np.expand_dims(imgC, 1)
        imgD = np.expand_dims(imgD, 1)
        imgALL = np.concatenate((imgA, imgB, imgC, imgD), axis=1)

        fnB = fn.replace("{}.jpg".format(index), "{}.jpg".format(index+4))
        img = Image.open(fnB).convert('RGB')  # 按照path读入图片from PIL import Image # 按照路径读取图片
        imgB = self.transform(img)  # 是否进行transform
        fnC = fn.replace("{}.jpg".format(index), "{}.jpg".format(index+5))
        img = Image.open(fnC).convert('RGB')  # 按照path读入图片from PIL import Image # 按照路径读取图片
        imgC = self.transform(img)  # 是否进行transform
        fnD = fn.replace("{}.jpg".format(index), "{}.jpg".format(index+6))
        img = Image.open(fnD).convert('RGB')  # 按照path读入图片from PIL import Image # 按照路径读取图片
        imgD = self.transform(img)  # 是否进行transform
        fnE = fn.replace("{}.jpg".format(index), "{}.jpg".format(index+7))
        img = Image.open(fnE).convert('RGB')  # 按照path读入图片from PIL import Image # 按照路径读取图片
        imgE = self.transform(img)  # 是否进行transform
        imgB = np.expand_dims(imgB, 1)
        imgC = np.expand_dims(imgC, 1)
        imgD = np.expand_dims(imgD, 1)
        imgE = np.expand_dims(imgE, 1)
        imgALL = np.concatenate((imgALL, imgB, imgC, imgD, imgE), axis=1)

        fnB = fn.replace("{}.jpg".format(index), "{}.jpg".format(index+8))
        img = Image.open(fnB).convert('RGB')  # 按照path读入图片from PIL import Image # 按照路径读取图片
        imgB = self.transform(img)  # 是否进行transform
        fnC = fn.replace("{}.jpg".format(index), "{}.jpg".format(index+9))
        img = Image.open(fnC).convert('RGB')  # 按照path读入图片from PIL import Image # 按照路径读取图片
        imgC = self.transform(img)  # 是否进行transform
        fnD = fn.replace("{}.jpg".format(index), "{}.jpg".format(index+10))
        img = Image.open(fnD).convert('RGB')  # 按照path读入图片from PIL import Image # 按照路径读取图片
        imgD = self.transform(img)  # 是否进行transform
        fnE = fn.replace("{}.jpg".format(index), "{}.jpg".format(index+11))
        img = Image.open(fnE).convert('RGB')  # 按照path读入图片from PIL import Image # 按照路径读取图片
        imgE = self.transform(img)  # 是否进行transform
        imgB = np.expand_dims(imgB, 1)
        imgC = np.expand_dims(imgC, 1)
        ...
        fnB = fn.replace("{}.jpg".format(index), "{}.jpg".format(index+12))
        img = Image.open(fnB).convert('RGB')  # 按照path读入图片from PIL import Image # 按照路径读取图片
        imgB = self.transform(img)  # 是否进行transform
        fnC = fn.replace("{}.jpg".format(index), "{}.jpg".format(index+13))
        img = Image.open(fnC).convert('RGB')  # 按照path读入图片from PIL import Image # 按照路径读取图片
        imgC = self.transform(img)  # 是否进行transform
        fnD = fn.replace("{}.jpg".format(index), "{}.jpg".format(index+14))
        img = Image.open(fnD).convert('RGB')  # 按照path读入图片from PIL import Image # 按照路径读取图片
        imgD = self.transform(img)  # 是否进行transform
        fnE = fn.replace("{}.jpg".format(index), "{}.jpg".format(index+15))
        img = Image.open(fnE).convert('RGB')  # 按照path读入图片from PIL import Image # 按照路径读取图片
        imgE = self.transform(img)  # 是否进行transform
        imgB = np.expand_dims(imgB, 1)
        imgC = np.expand_dims(imgC, 1)
        ...
        img = imgALL
        # img = np.concatenate((img, img), axis=1)        # 32
        # img = np.concatenate((img, img), axis=3)
        # img = np.concatenate((img, img), axis=3)      # 128
...
def get_dataloaderYXY(batch_size, root='./data_11_CT/'):
...
#                     transforms.ToTensor(),  # 转为张量，同时归一化
#                     # transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225)),  # 标准化      #原文是transforms.Normalize(norm_mean, norm_std),
#                     transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),       # CXY的
#                     ]), root='./data_11_CT/')
#
#     # val_data = MyDataset(datatxt=root + 'data_2_val.txt', transform = transforms.Compose([transforms.Resize((256, 256)),  # 缩放
#     val_data = MyDataset(datatxt=root + 'data_2_val.txt', transform =  transforms.Compose([transforms.Resize((256, 256)),  # 缩放
#                     transforms.CenterCrop((224, 224)),  # 缩放
#                     # transforms.RandomCrop(224, padding=4),  # 裁剪
#                     # transforms.RandomCrop(224, padding=None),  # 裁剪
#                     # transforms.RandomHorizontalFlip(),  #**功能：**依据概率p对PIL图片进行水平翻转，p默认0.5
#                     transforms.ToTensor(),  # 转为张量，同时归一化
...
#     test_data = MyDataset(datatxt=root + 'data_2_test.txt', transform = transforms.Compose([transforms.Resize((224, 224)),      # 中心裁切
#                     # transforms.RandomCrop(224, padding=None),  # 裁剪
#                     transforms.ToTensor(),  # 转为张量，同时归一化
#                     # transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225)),  # 标准化      #原文是transforms.Normalize(norm_mean, norm_std),
#                     transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),
...
#下面这3个就是参考的上面的：https://blog.csdn.net/haozige888/article/details/108268042分享技术图片预处理 transforms 模块机制
    # train_data = MyDataset_Rand(datatxt=root + 'data_2_train.txt', transform = transforms.Compose([transforms.Resize((256, 256)),  # 缩放
    train_data = MyDataset(datatxt=root + 'data_2_train.txt', transform = transforms.Compose([transforms.Resize((256, 256)),  # 缩放
                    transforms.RandomCrop(224, padding=None),  # 裁剪
                    # transforms.CenterCrop((224, 224)),  # 缩放
                    transforms.RandomHorizontalFlip(),  #**功能：**依据概率p对PIL图片进行水平翻转，p默认0.5
                    transforms.RandomVerticalFlip(),      # 依据概率p对PIL图片进行垂直翻转
    ...
    # val_data = MyDataset(datatxt=root + 'data_2_val.txt', transform = transforms.Compose([transforms.Resize((224, 224)),  # 缩放
    val_data = MyDataset(datatxt=root + 'data_2_val.txt', transform =  transforms.Compose([transforms.Resize((256, 256)),  # 缩放
                    # transforms.RandomCrop(224, padding=None),  # 裁剪
                    transforms.CenterCrop((224, 224)),  # 缩放
                    # transforms.RandomCrop(224, padding=None),  # 裁剪
    ...
    # test_data = MyDataset(datatxt=root + 'data_2_test.txt', transform = transforms.Compose([transforms.Resize((224, 224)),  # 缩放
    test_data = MyDataset(datatxt=root + 'data_2_test.txt', transform = transforms.Compose([transforms.Resize((256, 256)),  # 缩放
                    # transforms.RandomCrop(224, padding=None),  # 裁剪
                    transforms.CenterCrop((224, 224)),  # 缩放transforms.RandomCrop(224, padding=None),  # 缩放裁剪
                    transforms.ToTensor(),  # 转为张量，同时归一化
                    # transforms.Normalize((0.485, 0.456, 0.406), (0.229, 0.224, 0.225)),  # 标准化      #原文是transforms.Normalize(norm_mean, norm_std),
                    transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),
                    ]), root='./data_11_CT/')
...
#     test_loader = DataLoader(dataset=test_data, batch_size=batch_size, shuffle=True, num_workers=4)
    ...
    # train_loader = DataLoaderX(dataset=train_data, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True)
    # val_loader = DataLoaderX(dataset=val_data, batch_size=batch_size, shuffle=True, num_workers=4,pin_memory=True) #batch_size单次训练用的样本数            shuffle:先对batch内打乱,再按顺序取batch
    # test_loader = DataLoaderX(dataset=test_data, batch_size=batch_size, shuffle=True, num_workers=4,pin_memory=True)

    train_loader = DataLoaderX(dataset=train_data, batch_size=batch_size, shuffle=True, num_workers=2, pin_memory=True)
    val_loader = DataLoaderX(dataset=val_data, batch_size=batch_size, shuffle=True, num_workers=2, pin_memory=True) #batch_size单次训练用的样本数            shuffle:先对batch内打乱,再按顺序取batch
    test_loader = DataLoaderX(dataset=test_data, batch_size=batch_size, shuffle=False, num_workers=2,pin_memory=True)

    # train_loader = DataLoader(dataset=train_data, batch_size=batch_size, shuffle=True, num_workers=2, pin_memory=True)
    # val_loader = DataLoader(dataset=val_data, batch_size=batch_size, shuffle=True, num_workers=2, pin_memory=True) #batch_size单次训练用的样本数            shuffle:先对batch内打乱,再按顺序取batch
    # test_loader = DataLoader(dataset=test_data, batch_size=batch_size, shuffle=True, num_workers=2,pin_memory=True)
