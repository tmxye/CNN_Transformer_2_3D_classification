...
from .layers import ModuleInjection, Attention, SparseAttention
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, head_search=False, uniform_search=False, num_patches=197):
        super().__init__()
        ...
        self.attn = Attention(
            # 197 198
            dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, num_patches=198)
            # dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, num_patches=197)
    ...
    def forward(self, x):
        ...
        # by YXY
        # x = x + self.drop_path(self.mlp(self.norm2(x) * z_patch))
        # print("z_patch", z_patch.shape)
        # print("x", x.shape)
        xx = self.norm2(x) * z_patch
        x = x + self.drop_path(self.mlp(xx))
        return x
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.blocks = nn.Sequential(*[
            Block(
                ...
                head_search=head_search, uniform_search=uniform_search, num_patches=self.num_patches)
