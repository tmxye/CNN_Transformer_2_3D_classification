...
class SwinTransformer_b(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.norm = norm_layer(self.num_features)
        self.avgpool = nn.AdaptiveAvgPool1d(1)
        self.headA = nn.Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()
...
def SwinTransformer_b_pre(pretrained=False, in_22k=False, num_classes=1000, **kwargs):
    model = SwinTransformer_b(num_classes=num_classes, **kwargs)
    if pretrained:
        print("预处理开始下载")
