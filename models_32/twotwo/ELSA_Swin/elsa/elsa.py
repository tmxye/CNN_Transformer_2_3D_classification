...
class ELSAFunctionCUDA(Function):
    ...
    def forward(ctx, features, ghost_mul, ghost_add, h_attn,
        ...
        # print("features", features.shape)     # ([3, 32, 56, 56])
        # print("ghost_mul", ghost_mul.shape)     # ([3, 32, 7, 7])
        # print("ghost_add", ghost_add.shape)     # e([3, 32, 7, 7])
        # print("h_attn", h_attn.shape)       # ([3, 49, 56, 56])
        ...
        # print("features", features.shape)     # ([3, 32, 56, 56])
        # print("ghost_mul", ghost_mul.shape)     # ([3, 32, 7, 7])
        # print("ghost_add", ghost_add.shape)     # ([3, 32, 7, 7])
        # print("h_attn", h_attn.shape)       # ([3, 49, 56, 56])
        # print("output", output.shape)       # ([3, 32, 56, 56])
        # features torch.Size([12, 32, 14, 14])
        # ghost_mul torch.Size([12, 32, 7, 7])
        # ghost_add torch.Size([12, 32, 7, 7])
        # h_attn torch.Size([12, 49, 14, 14])
        # features torch.Size([12, 32, 14, 14])
        # ghost_mul torch.Size([12, 32, 7, 7])
        # ghost_add torch.Size([12, 32, 7, 7])
        # h_attn torch.Size([12, 49, 14, 14])
        # output torch.Size([12, 32, 14, 14])
        return output
...
def elsa_op(features, ghost_mul, ghost_add, h_attn, lam, gamma,
    ...
    if features.is_cuda and ghost_mul.is_cuda and ghost_add.is_cuda and h_attn.is_cuda:
        ...
        print("features", features.shape)     # V torch.Size([3, 32, 56, 56])
        print("ghost_mul", ghost_mul.shape)
        print("ghost_add", ghost_add.shape)
        ...
        print("(features * filters)", (features * filters).shape)
        print("(features * filters).sum(2)", (features * filters).sum(2).shape)
        print("(features * filters).sum(2).reshape(B, C, H, W)", (features * filters).sum(2).reshape(B, C, H, W).shape)
...
class ELSA(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        ks = self.kernel_size
        G = self.num_heads
        # print("x", x.shape)         # ([1, 56, 56, 96])
        x = x.permute(0, 3, 1, 2)  # B, C, H, W
        # print("x", x.shape)     # ([1, 96, 56, 56])

        qkv = self.pre_proj(x)
        # print("qkv", qkv.shape)     # qkv torch.Size([1, 224, 56, 56])

        q, k, v = torch.split(qkv, (self.dim_qk, self.dim_qk, self.dim_v), dim=1)
        # q torch.Size([1, 64, 56, 56])
        # k torch.Size([1, 64, 56, 56])
        # v torch.Size([1, 96, 56, 56])
        hadamard_product = q * k * self.scale
        # print("hadamard_product", hadamard_product.shape)       # hadamard_product torch.Size([1, 64, 56, 56])

        if self.stride > 1:
            hadamard_product = F.avg_pool2d(hadamard_product, self.stride)

        h_attn = self.attn(hadamard_product)
        # print("h_attn", h_attn.shape)       # h_attn torch.Size([1, 147, 56, 56])

        v = v.reshape(B * G, C // G, H, W)
        # print("V", v.shape)     # V torch.Size([3, 32, 56, 56])
        # h_attn.reshape(B * G, -1, H, W) torch.Size([3, 49, 56, 56])
        h_attn = h_attn.reshape(B * G, -1, H, W).softmax(1)
        # h_attn = h_attn.reshape(B, -1, H, W).softmax(1)
        # h_attn.shape) torch.Size([3, 49, 56, 56])
        # h_attn = torch.cat([h_attn, h_attn, h_attn, h_attn, h_attn, h_attn, h_attn], dim=1)           # ([16, 16, 14, 14, 98])
        ...
        if self.lam != 0 and self.gamma != 0:
            print("AAAAAAAAAA")
            print("self.ghost_head", self.ghost_head.shape)
            print("self.ghost_head.expand(2, B, C, ks, ks)", self.ghost_head.expand(2, B, C, ks, ks).shape)
            print("self.ghost_head.expand(2, B, C, ks, ks).reshape(2, B * G, C // G, ks, ks)", self.ghost_head.expand(2, B, C, ks, ks).reshape(2, B * G, C // G, ks, ks).shape)
            gh = self.ghost_head.expand(2, B, C, ks, ks).reshape(2, B * G, C // G, ks, ks)
            ghost_mul, ghost_add = gh[0], gh[1]
        elif self.lam == 0 and self.gamma != 0:
            # print("BBBBBBBBB")
            # self.ghost_head torch.Size([1, 96, 7, 7])
            # self.ghost_head.expand(2, B, C, ks, ks) torch.Size([1, 96, 7, 7])
            # self.ghost_head.expand(2, B, C, ks, ks).reshape(2, B * G, C // G, ks, ks) torch.Size([3, 32, 7, 7])
            ghost_add = self.ghost_head.expand(B, C, ks, ks).reshape(B * G, C // G, ks, ks)
            # ghost_add = self.ghost_head.expand(B, C, ks, ks).reshape(B, C, ks, ks)
        elif self.lam != 0 and self.gamma == 0:
            print("CCCCCCCCCCC")
            print("self.ghost_head", self.ghost_head.shape)
            print("self.ghost_head.expand(2, B, C, ks, ks)", self.ghost_head.expand(B, C, ks, ks).shape)
            print("self.ghost_head.expand(2, B, C, ks, ks).reshape(2, B * G, C // G, ks, ks)", self.ghost_head.expand(B, C, ks, ks).reshape(B * G, C // G, ks, ks).shape)
        ...
        # print("x", x.shape)     # x torch.Size([3, 32, 56, 56])
        x = x.reshape(B, C, H // self.stride, W // self.stride)
        # print("x", x.shape)     # x torch.Size([1, 96, 56, 56])
